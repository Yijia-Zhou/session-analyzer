...
- Add more real-data fixtures for newly observed MCP, dynamic tool, hook, approval, image generation, or collaboration payload variants. Synthetic lifecycle coverage now protects grouped labels and status for these families, but the current local corpus still lacks real dynamic tool, hook, approval, and image-generation protocol rows. Those unobserved payload shapes intentionally keep bounded request/response fallback summaries until real payloads justify field-specific presentation. / 为新观察到的 MCP、dynamic tool、hook、approval、图像生成或协作 payload variant 增加更多真实数据 fixture。Synthetic 生命周期覆盖现已保护这些事件族的分组 label 和 status，但当前本地语料仍缺少真实 dynamic tool、hook、approval 和图像生成协议行。这些尚未观察到的 payload 形态会有意继续使用受限的 request/response 兜底摘要，直到真实 payload 足以支持字段级展示。
