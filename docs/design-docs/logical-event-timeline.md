...
Known follow-up: locale propagation must include state-returning project job responses, not only ordinary GET read APIs, and raw-record DTO labels are display fields that should be localized without changing `recordType`, `payloadType`, `sourceRecordType`, or `sourceEventType`. The zh-CN catalog may keep widely accepted technical terms when clearer, but catalog values should not remain English merely because a namespace has not been translated. / 已知后续项：locale 传递必须覆盖会返回 state 的 project job 响应，而不只是普通 GET 只读 API；raw-record DTO label 属于展示字段，应被本地化，同时不改变 `recordType`、`payloadType`、`sourceRecordType` 或 `sourceEventType`。zh-CN catalog 可以在更清晰时保留广泛接受的技术英文词，但 catalog 值不应仅因为某个 namespace 尚未翻译而保持英文。
