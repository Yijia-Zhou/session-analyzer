...
- Last updated: 2026-07-10 / 最近更新：2026-07-10
- Related spec: / 相关规格：
...
The browser owns explicit `searchScope`, `searchQuery`, and `searchFilters = { file, kind, status }` state; the input element is only the editable free-text projection and never parses operators. `layerId` remains the single application event-layer state. A single-line HUD projects scope, Layer, filter count, compact scope metrics, and session navigation, while a non-modal parameter popover owns scope switching, editable filter rows, scope-and-Layer-aware suggestions, full count breakdowns, and the authoritative Clear all action. Its Layer shortcut closes the popover and focuses the existing topbar selector instead of introducing a second selector or state. Current-session compact metrics show only `current / discovered jump targets`; full-text occurrences and their growth explanation stay in the popover. Project compact metrics show matching sessions, the popover and result summary show matching session/event totals, and previous/next controls are absent. Ordinary session rows, project result rows, timeline data, suggestions, and target discovery use separate request generations and committed context keys. A project expression edit starts an explicit pending transition before debounce: it invalidates the previous request generation, clears cards and aggregate totals, hides the result summary, and renders Searching until `projectSearchDataContextKey()` commits. Session-scope query edits refresh timeline find state only; session-scope structured-filter edits reload only the selected timeline; project-scope expression edits reload only project results and never auto-select a card. Project-result drill-down captures the project result context, switches explicitly to session scope, loads a contiguous timeline prefix through `latestEvent.timelineIndex + 1` in chunks no larger than 500, and stops if scope, expression, layer, session, or locale changes before the prefix commits.

浏览器拥有显式的 `searchScope`、`searchQuery` 和 `searchFilters = { file, kind, status }` 状态；输入框元素只是可编辑自由文本投影，绝不解析操作符。`layerId` 仍是唯一的应用事件层状态。单行 HUD 投影范围、层级、筛选数、紧凑范围计数和 session 导航；非模态参数弹层负责范围切换、可编辑筛选行、感知范围与层级的候选、完整计数明细，以及唯一权威的全部清除动作。弹层中的层级快捷入口会关闭弹层并聚焦现有顶部选择器，不会引入第二个选择器或状态。当前 session 的紧凑计数只显示“当前位置 / 已发现跳转目标”；全文 occurrence 及其可能增长的说明保留在弹层中。项目紧凑计数显示匹配 session 数，弹层与结果摘要显示匹配 session/event 总数，并且不出现上一个/下一个控件。普通 session 行、项目结果行、timeline 数据、候选和目标发现分别使用独立的请求 generation 与已提交 context key。项目表达式编辑会在 debounce 前启动明确的 pending 转换：使上一请求 generation 失效、清空卡片和聚合计数、隐藏结果摘要，并显示“搜索中”，直到 `projectSearchDataContextKey()` 提交。Session 范围的 query 编辑只刷新 timeline 查找状态；session 范围的结构化筛选编辑只重载所选 timeline；项目范围的表达式编辑只重载项目结果，绝不自动选择卡片。项目结果下钻会捕获项目结果 context，显式切换到 session 范围，以不超过 500 条的 chunk 连续加载到 `latestEvent.timelineIndex + 1`，并在前缀提交前若范围、表达式、事件层、session 或 locale 变化就停止。
...
Frontend highlight rendering remains DOM-oriented, but navigation membership does not. In current-session scope, after HTML has been rendered, the browser walks only visible timeline events and the selected-event inspector. It skips nested interactive, hidden, or unsafe nodes and inserts `<mark>` elements with `textContent` around complete phrase matches instead of rewriting HTML strings. Session rows, project result cards, Raw refs, the empty-detail folding editor, search controls, summaries, and other UI chrome are outside session-target discovery. Project result cards use separate highlight markup for snippets and never register descriptors. Matching remains confined to each individual DOM text node; it does not concatenate text across HTML nodes. The displayed position comes from the stable descriptor registry described below, while backend `searchMatchCount` remains separate full-text context for unloaded or unrendered content. Navigation resolves a descriptor back to a concrete live mark, opens closed supplemental `<details>` ancestors, and then scrolls; ordinary highlight refresh remains passive.
...
The implemented architecture separates ephemeral highlight nodes from a stable navigation registry. A descriptor exists only for current-session navigation and contains the complete semantic search key (project, selected session, layer, free text, structured filters, folding-profile context, timeline sort order, and locale), surface (`timeline` or `inspector`), owner ID, and zero-based occurrence within that owner. The folding-profile context combines the profile ID with a deterministic serialization of normalized active rules, including unsaved draft rules; draft edits, cancellation, and same-ID saves therefore invalidate incompatible descriptors without relying on per-action reset calls. Its JSON tuple is the target identity, and `activeTargetId` stores the position independently of live nodes. Refresh clears bindings, re-highlights the searchable owners, rebinds matching identities, and appends new descriptors in discovery order; it never deletes descriptors for an unchanged key. Returning to a stale Inspector target loads its owner if needed, renders it with `searchTransient` provenance, resolves the same occurrence, and reveals it. A persisted manual fold remains authoritative, so a registered body descriptor can be temporarily unresolvable without being removed from the registry. / 已实现的架构会把短生命周期高亮节点与稳定导航注册表分开。Descriptor 只存在于当前 session 导航中，包含完整语义搜索 key（project、选中 session、layer、自由文本、结构化筛选、折叠 profile 上下文、timeline 排序和 locale）、surface（`timeline` 或 `inspector`）、owner ID，以及该 owner 内从零开始的 occurrence。折叠 profile 上下文由 profile ID 与规范化当前规则（包括未保存草稿规则）的确定性序列化组合而成；因此草稿编辑、取消和同 ID 保存都会让不兼容 descriptor 失效，无需依赖各操作入口分别执行 reset。其 JSON tuple 就是目标身份，`activeTargetId` 独立于实时节点保存当前位置。刷新会清空绑定、重新高亮可搜索 owner、按身份重新绑定，并按发现顺序追加新 descriptor；对未变化 key 永不删除 descriptor。返回失效的 Inspector 目标时，会按需加载 owner，以 `searchTransient` 来源渲染，解析同一 occurrence 并揭示它。持久化的手动折叠仍然具有更高优先级，因此已注册的正文 descriptor 可以暂时无法解析，但不会从注册表中删除。
...
Search-assist visibility is an explicit interaction state. Focus and click are equivalent open intents, including clicks on an already-focused input; search-driven scrolling is guarded from dismissing the assist or triggering automatic pagination, while user scrolling outside the focused field can still dismiss it. Wheel, touch, pointer, or scrolling-key input in the timeline explicitly ends that programmatic guard and queues a post-input pagination check, so a user takeover cannot lose the bottom-of-timeline load transition. The HUD remains one line at every supported width. When the search field has less than roughly 460 px, it hides only the compact metric text while retaining scope, Layer, filter count, input, and session navigation; at 760 px and below the labels shorten and the popover becomes viewport-bounded and scrollable. Escape closes file suggestions, the add-filter menu, or an empty filter editor before it closes the popover. / 搜索辅助面板可见性是显式交互状态。Focus 和 click 表达等价的打开意图，包括点击已经聚焦的输入框；搜索驱动滚动由 guard 阻止关闭辅助面板或触发自动分页，而焦点已离开输入框时的用户滚动仍可关闭它。时间线内的滚轮、触摸、指针或滚动键输入会明确结束程序化 guard，并排队一次输入后的分页检查，因此用户接管滚动时不会遗漏时间线底部的加载转换。HUD 在所有支持宽度下都保持单行。当搜索区域不足约 460 px 时，只隐藏紧凑计数文本，仍保留范围、层级、筛选数、输入和 session 导航；在 760 px 及以下会缩短标签，并让弹层受视口约束且可滚动。退出键会先关闭文件候选、添加筛选菜单或空筛选编辑器，之后才关闭弹层。
