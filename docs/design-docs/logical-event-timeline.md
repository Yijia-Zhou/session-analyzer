...
Frontend highlight rendering remains DOM-oriented, but navigation membership does not. After HTML has been rendered, the browser walks only searchable session rows, visible timeline events, and the selected-event inspector. It skips nested interactive, hidden, or unsafe nodes and inserts `<mark>` elements with `textContent` around complete phrase matches instead of rewriting HTML strings. Raw refs, the empty-detail folding editor, search controls, summaries, and other UI chrome are outside discovery. Matching remains confined to each individual DOM text node; it does not concatenate text across HTML nodes. The displayed position comes from the stable descriptor registry described below, while backend `searchMatchCount` remains separate full-text context for unloaded or unrendered content. Navigation resolves a descriptor back to a concrete live mark, opens closed supplemental `<details>` ancestors, and then scrolls; ordinary highlight refresh remains passive.

前端高亮渲染仍以 DOM 为单位，但导航成员关系不再由 DOM 决定。HTML 渲染完成后，浏览器只遍历可搜索的 session row、可见 timeline event 和选中事件 Inspector；嵌套的交互、隐藏或不安全节点会跳过，并用 `textContent` 为完整短语命中插入 `<mark>`，而不是重写 HTML 字符串。Raw refs、空详情态折叠编辑器、搜索控制、结果摘要和其它 UI chrome 都不参与发现。匹配仍限制在单个 DOM text node 内，不会跨 HTML 节点拼接文本。界面当前位置来自下文所述的稳定 descriptor 注册表；后端 `searchMatchCount` 继续作为独立全文上下文，用于提示尚未加载或尚未渲染的内容。导航会把 descriptor 重新解析成具体实时 mark，展开关闭的补充 `<details>` 祖先后再滚动；普通高亮刷新保持被动。
...
The implemented architecture separates ephemeral highlight nodes from a stable navigation registry. A descriptor contains the complete semantic search key (project, selected session, layer, free text, structured filters, folding-profile context, sort order, and locale), surface (`session`, `timeline`, or `inspector`), owner ID, and zero-based occurrence within that owner. The folding-profile context combines the profile ID with a deterministic serialization of normalized active rules, including unsaved draft rules; draft edits, cancellation, and same-ID saves therefore invalidate incompatible descriptors without relying on per-action reset calls. Its JSON tuple is the target identity, and `activeTargetId` stores the position independently of live nodes. Refresh clears bindings, re-highlights the searchable owners, rebinds matching identities, and appends new descriptors in discovery order; it never deletes descriptors for an unchanged key. Returning to a stale Inspector target loads its owner if needed, renders it with `searchTransient` provenance, resolves the same occurrence, and reveals it. A persisted manual fold remains authoritative, so a registered body descriptor can be temporarily unresolvable without being removed from the registry. / 已实现的架构会把短生命周期高亮节点与稳定导航注册表分开。Descriptor 包含完整语义搜索 key（project、选中 session、layer、自由文本、结构化筛选、折叠 profile 上下文、排序和 locale）、surface（`session`、`timeline` 或 `inspector`）、owner ID，以及该 owner 内从零开始的 occurrence。折叠 profile 上下文由 profile ID 与规范化当前规则（包括未保存草稿规则）的确定性序列化组合而成；因此草稿编辑、取消和同 ID 保存都会让不兼容 descriptor 失效，无需依赖各操作入口分别执行 reset。其 JSON tuple 就是目标身份，`activeTargetId` 独立于实时节点保存当前位置。刷新会清空绑定、重新高亮可搜索 owner、按身份重新绑定，并按发现顺序追加新 descriptor；对未变化 key 永不删除 descriptor。返回失效的 Inspector 目标时，会按需加载 owner，以 `searchTransient` 来源渲染，解析同一 occurrence 并揭示它。持久化的手动折叠仍然具有更高优先级，因此已注册的正文 descriptor 可以暂时无法解析，但不会从注册表中删除。

Descriptor discovery is also gated by committed render context. Session-list requests carry a context and request generation for repository, structured filters, layer, sort, and locale; timeline requests additionally carry selected session and free text. A response may update state only while both its generation and captured context still match. Session, timeline, and detail renders stamp their surface context, and discovery begins only when the current session and timeline surfaces are both committed. As a second defense, a DOM owner must still exist in the committed `sessions` or `currentEvents` collection; Inspector discovery additionally requires the committed detail owner and current timeline membership. Navigation, target preloading, and visible-detail prefetch remain disabled while this barrier is pending, and zero-size hidden timeline surfaces are excluded from visibility checks. / Descriptor 发现还受已提交渲染上下文的门控。Session 列表请求会携带覆盖仓库、结构化筛选、事件层、排序和 locale 的上下文及请求 generation；timeline 请求还会包含选中 session 和自由文本。只有 generation 与捕获上下文都仍匹配时，响应才能更新状态。Session、timeline 和 detail 渲染会记录各自的 surface 上下文，只有当前 session 与 timeline surface 都已提交后才开始发现。作为第二道防线，DOM owner 还必须存在于已提交的 `sessions` 或 `currentEvents` 集合中；Inspector 发现还要求已提交的 detail owner 和当前 timeline 成员关系。该屏障处于 pending 时，导航、目标预加载和可见详情预取保持禁用，并且尺寸为零的隐藏 timeline surface 不参与可见性判断。
