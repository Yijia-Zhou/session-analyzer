...
Search-assist visibility is an explicit interaction state. Focus and click are equivalent open intents, including clicks on an already-focused input; search-driven scrolling is guarded from dismissing the assist or triggering automatic pagination, while user scrolling outside the focused field can still dismiss it. Wheel, touch, pointer, or scrolling-key input in the timeline explicitly ends that programmatic guard and queues a post-input pagination check, so a user takeover cannot lose the bottom-of-timeline load transition. The HUD remains one line at every supported width and always projects scope as the concise `session` / `project` labels, while the parameter popover retains the full scope names. When the search field has less than roughly 460 px, it hides only the compact metric text while retaining scope, filter count, input, and session navigation; the adjacent canonical Layer selector remains outside the HUD. At 760 px and below the popover becomes viewport-bounded and scrollable. Escape closes file suggestions before it closes the popover. / 搜索辅助面板可见性是显式交互状态。Focus 和 click 表达等价的打开意图，包括点击已经聚焦的输入框；搜索驱动滚动由 guard 阻止关闭辅助面板或触发自动分页，而焦点已离开输入框时的用户滚动仍可关闭它。时间线内的滚轮、触摸、指针或滚动键输入会明确结束程序化 guard，并排队一次输入后的分页检查，因此用户接管滚动时不会遗漏时间线底部的加载转换。HUD 在所有支持宽度下都保持单行，并始终以简洁的 `session` / `project` 标签投影范围，参数弹层则保留完整范围名称。当搜索区域不足约 460 px 时，只隐藏紧凑计数文本，仍保留范围、筛选数、输入和 session 导航；相邻的唯一层级选择器位于 HUD 之外并继续保留。在 760 px 及以下，弹层受视口约束且可滚动。退出键会先关闭文件候选，之后再关闭弹层。
