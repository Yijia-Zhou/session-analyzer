...
- Last updated: 2026-06-30 / 最近更新：2026-06-30
- Related spec: / 相关规格：
...
Search-assist visibility is an explicit interaction state. Focus and click are equivalent open intents, including clicks on an already-focused input; search-driven scrolling is guarded from dismissing the assist or triggering automatic pagination, while user scrolling outside the focused field can still dismiss it. Wheel, touch, pointer, or scrolling-key input in the timeline explicitly ends that programmatic guard and queues a post-input pagination check, so a user takeover cannot lose the bottom-of-timeline load transition. The inline count container and text are pointer-transparent, but the actual previous/next buttons keep pointer events and accessible names. Layout code measures the unabridged localized control width, reserves that exact input padding when at least 120 px remains usable, and otherwise relocates the controls below the input. / 搜索辅助面板可见性是显式交互状态。Focus 和 click 表达等价的打开意图，包括点击已经聚焦的输入框；搜索驱动滚动由 guard 阻止关闭辅助面板或触发自动分页，而焦点已离开输入框时的用户滚动仍可关闭它。时间线内的滚轮、触摸、指针或滚动键输入会明确结束程序化 guard，并排队一次输入后的分页检查，因此用户接管滚动时不会遗漏时间线底部的加载转换。内联计数容器和文本对指针透明，真正的上一个/下一个按钮继续保留指针事件和无障碍名称。布局代码会测量不缩写的本地化控件宽度；若搜索框仍可保留至少 120 px 可用宽度，就精确预留对应 padding，否则把控件移到输入框下方。
