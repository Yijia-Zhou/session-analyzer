...
Frontend highlighting is DOM-oriented. After HTML has been rendered, the browser walks text nodes under the session list, timeline, and detail panel, skips interactive, hidden, or unsafe nodes such as inputs, buttons, links, profile-hidden events, scripts, styles, and existing marks, and inserts `<mark>` elements with `textContent` around complete phrase matches instead of rewriting HTML strings. Matching remains confined to each individual DOM text node; it does not concatenate text across HTML nodes. The displayed position uses the currently rendered and jumpable marks as the primary denominator, while the backend `searchMatchCount` remains visible as separate full-text context for unloaded or unrendered content. Previous/next navigation scrolls the concrete live mark after any synchronous inspector redraw; when that target is inside closed supplemental `<details>` sections, navigation opens every ancestor first. Ordinary highlight refresh remains passive and does not expand sections.

前端高亮则以 DOM 为单位。HTML 渲染完成后，浏览器遍历 session 列表、时间线和详情面板下的文本节点，跳过 input、button、link、被 profile 隐藏的事件、script、style 和已有 mark 等交互、隐藏或不适合处理的节点，并用 `textContent` 为完整短语命中插入 `<mark>`，而不是重写 HTML 字符串。匹配仍限制在单个 DOM text node 内，不会跨 HTML 节点拼接文本。界面显示的当前位置以当前已渲染且可跳转的 mark 作为主分母，同时把后端 `searchMatchCount` 作为独立的全文上下文显示，用于提示尚未加载或尚未渲染的内容。上一个/下一个导航会在 Inspector 的同步重绘完成后滚动实际存在的具体 mark；当目标位于关闭的补充 `<details>` 区段中时，导航会先展开其所有祖先。普通高亮刷新保持被动，不会展开区段。
...
The current browser implementation still derives three responsibilities from one mutable array of live DOM `<mark>` nodes: highlight rendering, previous/next navigation order, and the displayed jump-target denominator. `refreshSearchHighlights()` rebuilds that array by rescanning the session list, timeline, and detail panel, while `setActiveSearchMark()` preserves only a numeric index. Search navigation can call `showInspector()`, which redraws the detail panel and refreshes highlights again; timeline materialization can redraw and expand the timeline as well. Consequently, navigation side effects mutate the collection being navigated. In a reproduced search, an inspector redraw changed `1 / 18` to `2 / 17`, and later materialization changed `18 / 18` to `17 / 33`: legitimate discovery of new timeline targets was mixed with illegitimate removal and replacement of transient detail marks. / 当前浏览器实现仍让一个可变的实时 DOM `<mark>` 数组同时承担三项职责：高亮渲染、上一个/下一个导航顺序，以及界面显示的可跳目标分母。`refreshSearchHighlights()` 会重新扫描 session 列表、timeline 和 detail panel 来重建该数组，而 `setActiveSearchMark()` 只保留数值索引。搜索导航可能调用 `showInspector()`，后者会重绘 detail panel 并再次刷新高亮；timeline 目标物化也可能重绘并展开时间线。因此，导航副作用会反过来修改正在导航的集合。一次复现中，Inspector 重绘让 `1 / 18` 变为 `2 / 17`，随后目标物化又让 `18 / 18` 变为 `17 / 33`：合法的新 timeline 目标发现与不合法的临时 detail mark 删除和替换混在了一起。

The target architecture separates ephemeral highlight nodes from a stable navigation registry. Each jump target is represented by a semantic descriptor keyed to the active search target, its surface, its owning session or event, and its occurrence within that owner. The active position is stored by target identity rather than by array offset. DOM refresh rebinds known descriptors to live marks and may append genuinely newly materialized descriptors, but it must not shrink or replace the registry merely because an inspector rerendered. Inspector targets remain supported: returning to one rematerializes its owning event and resolves the concrete mark instead of depending on the previous node staying alive. Searchable content regions may contribute targets; folding controls, result summaries, and other UI chrome must not. / 目标架构会把短生命周期的高亮节点与稳定的导航注册表分开。每个可跳目标都用语义 descriptor 表示，并绑定到当前搜索目标、所属 surface、所属 session 或 event，以及该 owner 内的命中序号。当前位置按目标身份保存，而不是按数组偏移保存。DOM 刷新会把已知 descriptor 重新绑定到实时 mark，并且可以追加真正新物化的 descriptor，但不能仅因为 Inspector 重绘就缩小或替换注册表。Inspector 目标仍然保留：返回这类目标时，会重新物化其所属事件并解析到具体 mark，而不是依赖旧节点持续存活。可搜索内容区可以贡献目标；折叠控件、结果摘要和其它 UI chrome 不得贡献目标。

Navigation is an asynchronous state transition and must be serialized. Loading another page, expanding an event, rendering detail, refreshing highlights, opening supplemental `<details>`, and scrolling to the resolved mark complete as one transition before another previous/next action mutates the same state. The denominator may grow when that transition discovers previously unmaterialized content, but it must not oscillate because a transient surface changed, and the active target identity must survive redraws. / 导航是异步状态转换，必须串行执行。加载下一页、展开事件、渲染详情、刷新高亮、打开补充 `<details>`，以及滚动到解析后的 mark，应作为一次完整转换结束后，另一次上一个/下一个操作才能修改同一状态。若该转换发现此前尚未物化的内容，分母可以增长；但不能因为临时 surface 变化而来回波动，而且活动目标身份必须跨重绘保留。

Search-assist visibility is a separate interaction state, not an incidental consequence of focus and scroll events. Focusing or clicking the search input opens the assist, including when the input was already focused after the assist had closed. Programmatic scrolling initiated by search navigation must not close it while the input remains focused; timeline-scroll dismissal applies only to an actual user navigation context outside the focused search field. Inline count text is presentation-only and must not intercept pointer input intended for the search field, while the previous/next buttons retain their own hit areas. Hit testing and geometry are separate requirements: pointer transparency fixes click ownership, while the layout must reserve or responsively relocate the dynamic, unabridged count instead of assuming the current fixed `padding-right: 10rem` is always sufficient. The active remediation is tracked in `docs/exec-plans/active/2026-06-28-search-navigation-state-convergence.md`. / 搜索辅助面板的可见性是独立的交互状态，不能只是 focus 与 scroll 事件的偶然结果。聚焦或点击搜索框都会打开辅助面板，包括辅助面板关闭但输入框仍已聚焦的情况。由搜索导航发起的程序化滚动，在输入框仍保持焦点时不得关闭辅助面板；timeline scroll 关闭行为只适用于焦点已离开搜索框的真实用户导航场景。内联计数文本只负责展示，不得截获原本应落到搜索框的指针输入；上一个/下一个按钮则保留各自的点击区域。点击命中与几何布局是两个独立要求：指针穿透负责修复点击归属，布局则必须为动态且不缩写的计数预留空间或在响应式场景中重新安置它，不能假定当前固定的 `padding-right: 10rem` 始终足够。当前修复工作由 `docs/exec-plans/active/2026-06-28-search-navigation-state-convergence.md` 跟踪。
...
Search-result preloading is intentionally bounded. When free-text search has too few rendered jump targets, the browser may append a few more timeline pages so previous/next navigation has nearby real scroll targets. Explicit previous/next navigation is stronger than preload: before wrapping, it can load additional timeline pages until the next hit event is present or the filtered timeline is exhausted. When a loaded hit event has no live DOM mark, navigation may temporarily expand that event, load its detail, refresh marks, and activate the concrete mark in the timeline body. This transient expansion is keyed by session, layer, free text, kind, status, and file; it is cleared when that target changes, the session/layer/profile changes, free text is cleared, or the user manually overrides the same event. Persisted user overrides remain higher priority and are not replaced by transient expansion.

搜索结果预加载有意保持有限。当自由文本搜索下可渲染跳转目标过少时，浏览器可以追加加载少量时间线分页，让上一个/下一个导航有附近的真实滚动目标。明确的上一个/下一个导航强于预加载：在循环回到已渲染目标之前，它可以继续加载时间线分页，直到下一个命中事件出现或筛选后的时间线耗尽。当已加载的命中事件没有实时 DOM mark 时，导航可以临时展开该事件、加载详情、刷新 mark，并激活时间线正文里的具体命中。这个临时展开由 session、layer、自由文本、kind、status 和 file 绑定；当目标变化、session/layer/profile 变化、自由文本清空，或用户手动覆盖同一事件时清除。持久用户覆盖优先级更高，不会被临时展开替换。
