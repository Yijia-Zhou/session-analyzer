...
`Code Mode request / Code Mode 请求` is a separate Main-layer query and catalog dimension, not an Event type alias. The exact query parameter is `codeModeRequest=<stable-tool-name>`. It matches only canonical `subtype === 'code_mode_operation'` events carrying the requested safe fact, composes with `q`, `kind`, `status`, and `file` on the same parent event, and clears on a Protocol or Raw transition. In the browser filter surface it is not a peer control: the Kind select groups `Code Mode tool call` with an all-operations option and `Declared: <tool>` children. A child selection atomically projects to `kind=code_mode_operation` plus `codeModeRequest=<stable-tool-name>` and counts as one visible Kind filter; selecting any other Kind clears `codeModeRequest`. This is a presentation hierarchy only—the backend fields and their evidence semantics stay separate. A repeated tool declaration counts once per operation; a multi-tool operation can match several facets but remains one event in totals, pagination, phrase occurrences, matching-event counts, and jump targets. `status` and `file` keep reading canonical parent facts. State and timeline payloads expose `codeModeRequests` beside `eventKinds`; each row has stable `value`, localized `label`, operation `count`, and `evidence: 'declared_source'`. A friendly label never replaces the stable value, so colliding display labels remain distinguishable. Catalog counts are matching operations rather than call sites or execution counts, so facets may overlap.

`Code Mode request / Code Mode 请求` 是独立的 Main 层查询与目录维度，而不是 Event type 的别名。精确查询参数为 `codeModeRequest=<stable-tool-name>`。它只匹配带有所请求安全事实的 canonical `subtype === 'code_mode_operation'` 事件，并与 `q`、`kind`、`status` 和 `file` 在同一个父事件上组合；切换到 Protocol 或 Raw 时会清除。在浏览器筛选表面中，它不是平级控件：类型选择器把 `Code Mode 工具调用` 分成“全部 operation”选项与 `声明：<工具>` 子项。选择子项会原子投影为 `kind=code_mode_operation` 加 `codeModeRequest=<stable-tool-name>`，并只计作一个可见类型筛选；选择任何其他类型都会清除 `codeModeRequest`。这只是展示层级，后端字段及其证据语义仍相互独立。重复声明同一工具在每个 operation 中只计一次；多工具 operation 可以命中多个分面，但在总数、分页、短语 occurrence、匹配事件数和跳转 target 中仍是一个事件。`status` 与 `file` 继续读取 canonical 父事件事实。State 与 timeline payload 会在 `eventKinds` 旁暴露 `codeModeRequests`；每一行都有稳定的 `value`、本地化 `label`、按 operation 计的 `count` 和 `evidence: 'declared_source'`。友好 label 绝不取代稳定 value，因此发生展示 label 冲突时仍能区分。目录计数表示匹配 operation，而不是调用位置或执行次数，因此各分面可以重叠。

Folding profiles add the independent `codeModeRequestStates` rule family. Natural state resolution takes the canonical kind rule, every matching request rule, and every matching existing condition rule, then retains the most-visible merge `expanded > summary > collapsed > hidden`; a valid manual event override remains higher. Built-in profiles initially use no request rules, so their default visibility is unchanged. Missing maps normalize to `{}`, while valid unknown or historical tool keys survive browser-local normalization and persistence and appear under `Other / historical requests` until the current catalog exposes them again. Request-rule edits preview directly from timeline DTO facts and do not fetch detail. The editor presents these existing rules as one hierarchy without changing storage: a dedicated Code Mode card renders the `codeModeOperation` condition as the all-operation parent and `codeModeRequestStates` as declared-request children. The parent is omitted from the generic Conditions section, unset parent/child selects say `Inherit other rules`, current child rows show operation counts and stable ids, and draft differences receive row-level Changed markers. The Code Mode section heading is locally sticky within its section so a long request catalog keeps context in the narrow detail pane.

折叠策略增加独立的 `codeModeRequestStates` 规则族。自然状态求值会收集 canonical kind 规则、每一条命中的 request 规则和每一条命中的既有 condition 规则，然后继续使用最可见合并 `expanded > summary > collapsed > hidden`；合法的手动事件覆盖仍具有更高优先级。内置 profile 初始不包含 request 规则，因此默认可见性不变。缺失 map 会规范化为 `{}`；有效但未知或历史的工具 key 会跨浏览器本地规范化与持久化保留，并在当前目录重新暴露前显示于“其他/历史请求”。Request 规则编辑直接依据 timeline DTO 事实预览，不会请求 detail。编辑器在不改变存储结构的前提下把这些既有规则展示为一个层级：专属 Code Mode 卡片把 `codeModeOperation` condition 渲染为作用于全部 operation 的父规则，把 `codeModeRequestStates` 渲染为声明 request 子规则。父规则不再重复出现在通用“条件”区；未设置的父/子下拉显示“继承其他规则”，当前子行显示 operation 计数与稳定 id，草稿差异显示行级“已修改”标记。Code Mode 区标题在其区段内局部吸顶，使较长 request 目录在狭窄详情栏中滚动时仍保留上下文。

The profile header remains sticky above the scrolling editor so the active strategy, dirty status, and save/cancel actions do not disappear during long Code Mode catalogs. The card explains that its parent count is matching operations while its child badge is the number of declared request types; request-type groups can overlap because one operation may declare several requests.

Profile 标题会固定在滚动编辑器上方，使当前策略、未保存状态和保存/取消操作不会在较长的 Code Mode 目录中消失。卡片会说明父级计数表示匹配 operation 数，子级徽标表示声明 request 类型数；由于一个 operation 可以声明多种 request，各 request 类型分组可以重叠。
...
The descriptor and the name-only request fact are presentation-only. All three variants keep canonical `kind: other_tool_call`, `subtype: code_mode_operation`, `toolName: exec`, operation identity, metric/search/Raw-ref ownership, neutral outcome, and independently visible Observed Nested Activity. The bounded AST projector is no longer detail-only: cold indexing classifies only safely declared tool names into the independent presentation index, while visible Code Mode cards may still lazily fetch detail for structured arguments, source, and result-association evidence. / 该 descriptor 与仅名称 request 事实都只属于 presentation。三个 variant 都保持 canonical `kind: other_tool_call`、`subtype: code_mode_operation`、`toolName: exec`、operation identity、指标/搜索/Raw-ref 所有权、中性 outcome，以及独立可见的已观测嵌套活动。有界 AST projector 不再只在 detail 阶段运行：cold indexing 只把安全声明的工具名称分类到独立的 presentation index，而可见 Code Mode 卡片仍可惰性获取 detail，以展示结构化 arguments、source 与 result-association 证据。
...
- 2026-07-26: Unified the folding editor's Code Mode condition and request rules into one parent-child card, with inherited-state wording, operation counts, local sticky context, and row-level draft markers. Storage and most-visible rule evaluation remain unchanged. / 2026-07-26：把折叠编辑器中的 Code Mode condition 与 request 规则统一为一张父子卡片，并增加继承状态文案、operation 计数、区段内吸顶上下文和行级草稿标记；存储结构与最可见规则求值保持不变。
