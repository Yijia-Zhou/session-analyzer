...
Native DSH format 4 uses `tool/ptc-dispatch-start` / `tool/ptc-dispatch` with the same durable call identities; format 0 retains `tool/code-dispatch-start` / `tool/code-dispatch`. Both preserve native event names in Raw References. The outer tool remains `run_code`; v4 does not record a per-call language, so Detail leaves code highlighting unspecified. See [native format 4](deepseek-format4-compatibility.md). / DSH 原生 format 4 使用 PTC dispatch 标签，format 0 保留原 Code Mode 标签；两者按相同持久调用身份关联，Raw Reference 保留原生事件名。外层工具仍是 `run_code`；v4 未记录每次调用的语言，因此 Detail 不指定代码高亮语言。详见原生 format 4 设计。
...
The shared Code Mode registry supplies stable user-visible request titles: `exec_command` is `Exec command / Exec 命令`, `shell_command` is `Shell command / Shell 命令`, and `write_stdin` is `Write stdin / 写入标准输入`. This is presentation-only: machine values, `codeModeRequest` filter values, and ordinary-kind inheritance remain unchanged.

shared Code Mode registry 提供稳定的用户可见 request 标题：`exec_command` 显示为 `Exec command / Exec 命令`，`shell_command` 显示为 `Shell command / Shell 命令`，`write_stdin` 显示为 `Write stdin / 写入标准输入`。这只影响呈现：machine value、`codeModeRequest` 筛选 value 与普通 kind 继承均保持不变。

Codex Code Mode accepts the direct-emission shape `text(await tools.<known_tool>(<bounded literal>))` as a strict extension of the top-level projector grammar. It requires the whole `text` argument to be that awaited static call; wrappers, arithmetic, dynamic members, and control flow remain raw fallback. A valid literal `write_stdin` request reuses the terminal presentation boundary: omitted or empty `chars` is a poll, every nonempty string is input, and malformed or non-string `chars` receives generic declared-request presentation. This Code Mode path does not participate in the separately admitted native direct-tool origin relation. / Codex Code Mode 将 `text(await tools.<known_tool>(<受限 literal>))` 作为顶层 projector grammar 的严格扩展。它要求 `text` 的整个参数就是该 await 静态调用；wrapper、算术、动态 member 与控制流继续 raw fallback。合法 literal `write_stdin` request 复用 terminal 呈现边界：缺失或空 `chars` 是 poll，每个非空字符串是 input，malformed 或非字符串 `chars` 使用 generic declared-request 呈现。这个 Code Mode path 不参与单独准入的 native direct-tool origin relation。
...
Projection extraction runs during cold Session indexing for the bounded name-only fact and during detail construction for structured request presentation. Both paths share the exact outer-exec source lookup and the same eligibility entry point. It uses a versioned known-tool allowlist, a real JavaScript parser, and an all-or-nothing top-level sequential grammar with recursively bounded literal arguments. Result emission accepts direct `text(result)`, direct static `text(result.output)`, and the standard type-preserving `text(typeof result === "string" ? result : JSON.stringify(result))` wrapper, with every identifier required to name the same declared result. The output-member form permits only a non-computed, non-optional `.output` access on that identifier. The ambient runtime identifiers `tools`, `text`, and `JSON` must remain unshadowed by every declared result binding; otherwise the source uses raw fallback. One unsupported statement, tool, binding, argument, or emission shape makes the entire source use raw fallback; partial projections are forbidden because they can imply a complete execution list.

投影提取会在冷启动的 Session 索引阶段为有界的仅名称事实运行，并在构建 detail 时为结构化 request 呈现运行。两条路径共享精确的 outer-exec 源码查找和同一个资格入口。它使用版本化的已知工具 allowlist、真正的 JavaScript parser，以及全有或全无的顶层顺序语法与递归受限的 literal arguments。结果 emission 接受直接的 `text(result)`、直接且静态的 `text(result.output)`，以及标准的保类型包装 `text(typeof result === "string" ? result : JSON.stringify(result))`，且其中每个 identifier 都必须指向同一个已声明结果。output 成员形态只允许该 identifier 上非 computed、非 optional 的 `.output` 访问。环境运行时标识符 `tools`、`text` 与 `JSON` 必须不被任何已声明的结果绑定遮蔽；否则该来源使用 raw fallback。只要存在一个不受支持的语句、工具、绑定、参数或 emission 形态，整个来源就使用 raw fallback；禁止生成局部投影，因为局部列表可能暗示它是一份完整执行清单。
...
The direct-awaited emission form `text(await tools.<known_tool>(<bounded literal>))` is an additional accepted result-emission shape. It requires exactly one `text` argument whose whole expression is the awaited static known-tool call; wrappers, arithmetic, dynamic members, and control flow remain raw fallback. Bounded association still requires every declared call and output fragment to align one-to-one in call order. / direct-awaited emission 形态 `text(await tools.<known_tool>(<受限 literal>))` 是额外接受的 result-emission 形态。它要求 `text` 恰好只有一个参数，且整个 expression 是 await 静态已知工具调用；wrapper、算术、动态 member 与控制流继续 raw fallback。bounded 关联仍要求每个 declared call 与 output fragment 按调用顺序一一对应。
