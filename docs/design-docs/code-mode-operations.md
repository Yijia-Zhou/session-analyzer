...
- Last updated: 2026-08-24 / 最近更新：2026-08-24
...
The shared Code Mode registry supplies stable user-visible request titles: `exec_command` is `Exec command / Exec 命令`, while `shell_command` is `Shell command / Shell 命令`. This is presentation-only: their machine values, the `codeModeRequest` filter values, and their shared ordinary `command` inheritance remain unchanged.

shared Code Mode registry 提供稳定的用户可见 request 标题：`exec_command` 显示为 `Exec command / Exec 命令`，而 `shell_command` 显示为 `Shell command / Shell 命令`。这只影响呈现：两者的 machine value、`codeModeRequest` 筛选 value，以及共同的普通 `command` 继承均保持不变。
