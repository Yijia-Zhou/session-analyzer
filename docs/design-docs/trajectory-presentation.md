...
Direct selection from the overview or narrative updates the canonical selection, selected locator, narrative state, and Inspector, materializing a containing Tool Activity Group when needed. It is not a navigation command and does not change the vertical `timelinePane` scroll position. Explicit search, Inspector previous/next, project drill-down, linked-event, and Raw Reference navigation retain their existing loading and scrolling paths. / 从 overview 或 narrative 直接选择 event 时，会更新 canonical selection、selected locator、narrative state 与 Inspector，并在需要时 materialize 所属工具活动组。该操作不是导航命令，不改变 `timelinePane` 的纵向 scroll position。显式 search、Inspector previous／next、project drill-down、linked-event 与原始引用导航继续使用既有 loading 与 scrolling path。

The compact Trajectory ledger does not duplicate full event cards, so its selected-event Inspector renders existing Primary Detail (`timelineSections`) through the existing Timeline section renderer before existing Supplemental Detail (`inspectorSections`), metadata, source provenance, and shared Raw References. Timeline keeps the accepted responsibility split: Primary Detail belongs to an expanded card and its Inspector remains Supplemental. Presentation switching re-renders only the current Inspector presentation; detail producers, API payloads, loading transactions, stale ownership, canonical search targets, and event identity remain unchanged. / 紧凑 Trajectory ledger 不复制完整 event card，因此其 selected-event Inspector 会先通过既有 Timeline section renderer 渲染现有主体详情（`timelineSections`），再渲染现有补充详情（`inspectorSections`）、metadata、source provenance 与共享原始引用。Timeline 保持已接受的职责划分：主体详情属于展开 card，Inspector 继续只承担补充详情。切换呈现时只重新渲染当前 Inspector presentation；detail producer、API payload、loading transaction、stale ownership、canonical search target 与 event identity 均不改变。
...
The overview is the persistent exploration control for the Trajectory ledger. Native CSS sticky positioning keeps only `.trajectoryOverview` at the top of the active reading viewport while the narrative scrolls: the desktop owner is the `timelinePane` scrollport, while the existing narrow Events flow uses document scrolling and places the overview below its sticky mobile tabs. The Session header, Main Presentation selector, Analysis Panel, and other surrounding controls remain ordinary scrolling content. The opaque surface and low-emphasis separation prevent narrative rows from showing through it. Sticky containment belongs to `.trajectoryPresentation`, so the overview releases normally at the end of Trajectory content. No scroll listener, persistent preference, or server state participates. / Overview 是 Trajectory ledger 的持续探索控制面。原生 CSS sticky positioning 只让 `.trajectoryOverview` 在 narrative 滚动时停留于当前阅读 viewport 顶部：desktop owner 是 `timelinePane` scrollport，既有窄屏 Events flow 则使用 document scrolling，并把 overview 放在 sticky mobile tabs 下方。Session header、Main Presentation selector、Analysis Panel 与其它周边控件仍是普通滚动内容。不透明 surface 与低强调分隔避免 narrative row 从其下方透出或混叠。Sticky containment 属于 `.trajectoryPresentation`，因此 overview 会在 Trajectory 内容末尾正常释放。不使用 scroll listener、持久 preference 或 server state。

Direct overview selection still preserves vertical scroll. Presentation continuity and explicit search, Inspector, project, linked-event, and Raw navigation may continue to center the selected narrative event. Their selected target must remain below the sticky overview rather than being obscured; the existing center-oriented reveal is preferred over path-specific navigation exceptions. Vertical narrative scrolling never changes Sequence Zoom or horizontal pan state. / 直接从 overview 选择仍保持纵向 scroll。Presentation continuity 以及显式 search、Inspector、project、linked-event 与 Raw navigation 可以继续把已选 narrative event 定位到中部。定位后的 selected target 必须处于 sticky overview 下方且不被遮挡；优先复用既有 center-oriented reveal，而不是增加逐路径 navigation 例外。Narrative 的纵向滚动绝不改变 Sequence Zoom 或横向 pan state。

The overview uses a bounded canvas plus a constant-size selected-event locator rather than one permanent DOM marker per event. The canvas width is bounded by the viewport and the supported zoom range, not by event count. Individual markers are fixed `3×8` CSS-pixel vertical ticks, while Unknown/Other events retain a cross-lane fail-safe mark. The selected locator line spans lanes lightly, remains visible at Fit even for 1,000+ events, and carries the canonical selected event ID; its knob aligns to the selected Input, Model, or Tools lane, with Other using the center fallback. / Overview 使用有界 canvas 与固定数量的 selected-event locator，而不是为每条 event 长期保留一个 DOM marker。Canvas 宽度由 viewport 和受支持 zoom range 限定，而不随 event 数增长。Individual marker 是固定 `3×8` CSS pixel 的竖向 tick，Unknown／Other event 则继续使用跨 lane fail-safe mark。Selected locator line 轻量贯穿 lanes，即使在 1000+ event 的 Fit 状态也保持可见，并携带 canonical selected event ID；其 knob 与已选 Input、Model 或 Tools lane 对齐，Other 使用中心 fallback。
...
Pointer selection follows the rendered encoding. In marker mode it selects solely by nearest canonical-sequence X position; lane Y never causes a horizontal jump, and Other needs no special precedence. In density mode, X first identifies the corresponding or nearest density bin. The clicked Input／Model／Tools lane is only a categorical preference among events inside that local bin; if the bin has no event in that lane, selection falls back to the globally X-nearest event. The algorithm uses no two-dimensional distance or continuous X/Y weighting. The narrative remains the accessible, identity-complete event list. / Pointer selection 遵循当前 rendering encoding。Marker mode 只按 canonical-sequence X 位置选择最近 event；lane Y 绝不会造成横向跳转，Other 也不需要特殊 precedence。Density mode 先由 X 确定对应或最近的 density bin；clicked Input／Model／Tools lane 只作为该 local bin 内 event 的 categorical preference；如果 bin 内没有该 lane 的 event，则回退到全局 X-nearest event。算法不使用二维距离，也不为 X／Y 定义连续权重。Narrative 继续作为具备无障碍能力且 identity 完整的 event list。
...
- Browser tests for selector/Layer memory, sticky overview geometry and containment on desktop and narrow screens, unobscured navigation targets, lane-aligned locator state, direct-selection scroll stability, presentation-specific Primary/Supplemental Detail responsibility, Raw refs, canonical search-target stability, search reveal inside a group, folding/filter refresh, project drill-down, previous/next, Load more, zoom/pan/free-pan behavior, loaded-prefix copy, locale, and stale context changes. / Browser test 覆盖 selector／Layer memory、desktop 与窄屏下的 sticky overview geometry 与 containment、不被遮挡的 navigation target、与 lane 对齐的 locator state、direct-selection scroll stability、presentation-specific 主体／补充详情职责、原始引用、canonical search-target stability、group 内 search reveal、folding／filter refresh、project drill-down、previous／next、Load more、zoom／pan／free-pan behavior、loaded-prefix 文案、locale 与 stale context change。
...
- 2026-09-05: Direct Trajectory selection became scroll-stable, the selected locator knob became lane-aware, marker hit-testing became X-nearest, density lane preference was bounded to the local X bin, the Trajectory Inspector assumed Primary Detail reading responsibility, and the overview became a native CSS-sticky persistent control surface without changing Timeline responsibility, navigation paths, detail producers, canonical search ownership, or server state. / 2026-09-05：Trajectory direct selection 改为保持纵向 scroll 稳定，selected locator knob 改为感知 lane，marker hit-testing 改为 X-nearest，density lane preference 被限制在 local X bin 内，由 Trajectory Inspector 承担主体详情阅读职责，并把 overview 改为原生 CSS sticky 的持续控制面，同时不改变 Timeline 职责、navigation path、detail producer、canonical search ownership 或 server state。
