...
- Snapshot date: 2026-09-11 / 快照日期：2026-09-11
...
- `text(await tools.<known_tool>(<bounded literal>));` for a direct final-output emission / 用于直接 final-output emission 的 `text(await tools.<known_tool>(<受限 literal>));`
...
- `bounded` is considered only for a single direct terminal exec phase with no waits. The outer output must be a typed text array whose first fragment is the canonical `Script completed` status-only envelope; all later non-empty fragments must match calls and accepted `text(variable)`, `text(variable.output)`, or direct `text(await tools.<known_tool>(...))` emissions one-to-one and in order. / 只有单个直接终态 exec phase 且没有 wait 时才考虑 `bounded`。外层输出必须是带类型的 text 数组，首个 fragment 必须是 canonical、仅包含状态的 `Script completed` envelope；之后的非空 fragments 必须与调用，以及已接受的 `text(variable)`、`text(variable.output)` 或 direct `text(await tools.<known_tool>(...))` emission 按顺序一一对应。
...
| Collaboration / 协作 | `spawn_agent`, `list_agents`, `wait_agent`, `send_message`, `send_input`, `followup_task`, `interrupt_agent`, `close_agent` | Targets, fields, statuses, message, and response-derived result in one collaboration card; `list_agents` consumes `agents[]` as count, statuses, and task messages / target、字段、状态、message 与响应派生 result 位于同一协作卡片；`list_agents` 会把 `agents[]` 消费为数量、状态与任务消息 | No generic response summary when the modeled card consumes the response; otherwise a bounded generic fallback; folded full associated result / 已建模卡片消费响应时不显示通用响应摘要，否则提供受限的通用回退；折叠完整关联结果 |
| Terminal continuation / 终端续接 | `write_stdin` | Valid literal requests show `Background terminal poll request` or `Background terminal input request` once in the header, followed by Process ID; nonempty input uses bounded JSON-string notation. Invalid `chars` uses the generic declared-request fallback. / 合法 literal request 在 header 显示一次“后台终端轮询请求”或“后台终端输入请求”，随后显示 Process ID；非空输入使用有界 JSON 字符串表示。无效 `chars` 使用 generic declared-request fallback。 | Recognized terminal metadata/output, generic response fallback for unknown shapes, and folded Associated result in single-tool Inspector; no successful-delivery or native-origin claim. / 可识别 terminal metadata／output，未知形态保留通用响应回退，single-tool Inspector 保留折叠的 Associated result；不声称输入成功送达，也不声称 native origin。 |
| Generic summary / 通用摘要 | Remaining nine tools / 其余九种工具 | Sanitized JSON/text `Request summary`, or a notice if empty / 脱敏 JSON/text `Request summary`，为空时显示 notice | Sanitized JSON/text `Response summary` plus folded full associated result / 脱敏 JSON/text `Response summary` 及折叠完整关联结果 |
...
### `write_stdin` — Write stdin / 写入标准输入

Presentation refinement: recognized typed terminal envelopes show metadata/output through the existing renderers; unknown shapes keep the response summary. The single-tool view keeps Associated result in Inspector, and states poll/input only in the header. This does not change projection admission or result association. / 展示调整：类型有效的 terminal envelope 通过既有 renderer 显示 metadata／output，未知形态保留响应摘要。Single-tool view 将 Associated result 保留在 Inspector，仅在 header 陈述 poll／input。不改变 projection 准入或 result association。

- **Current / 当前:** A direct `text(await tools.write_stdin(<literal>))` declaration is admitted by the closed-world projector. Missing or empty `chars` becomes `Background terminal poll request`; every nonempty string becomes `Background terminal input request` with process ID when valid and bounded JSON-string input. A bounded response uses the terminal presentation described above, retaining Associated result in Inspector for single-tool views. Invalid or non-string `chars` remains a generic declared-request projection, and this Code Mode path does not receive the native direct-tool origin relation. / 当前，直接的 `text(await tools.write_stdin(<literal>))` declaration 会被 closed-world projector 准入。缺失或空 `chars` 变成“后台终端轮询请求”；每个非空字符串变成“后台终端输入请求”，在 ID 合法时显示 process ID，并以有界 JSON 字符串显示输入。bounded response 使用上述 terminal 展示，single-tool view 将 Associated result 保留在 Inspector。无效或非字符串 `chars` 保持 generic declared-request projection；这个 Code Mode path 不获得 native direct-tool origin relation。
- **Fine-tune / 调优建议:** Keep request semantics source-specific and bounded. If future Code Mode evidence preserves a durable nested identity, evaluate a separate relation contract; do not reuse the native direct-tool relation solely because the tool name is the same. / 保持 request 语义来源专属且有界。如果未来 Code Mode evidence 保留 durable nested identity，再单独评估 relation contract；不能仅因工具名称相同就复用 native direct-tool relation。
- **Acceptance / 验收:** Empty and omitted `chars`, whitespace/newline/Ctrl-C input, malformed and non-string `chars`, bounded and unassociated output, dynamic arguments, wrappers, arithmetic, control flow, request catalogs, folding, canonical counts, Timeline/Detail, and Trajectory identity preservation. / 覆盖空／缺失 `chars`、空白／换行／Ctrl-C 输入、malformed 与非字符串 `chars`、bounded 与未关联 output、动态 arguments、wrapper、算术、控制流、request catalog、折叠、canonical 计数、Timeline/Detail，以及 Trajectory identity 保持。
- **Decision / 决定:** `Accepted baseline / 已接受基础版`
