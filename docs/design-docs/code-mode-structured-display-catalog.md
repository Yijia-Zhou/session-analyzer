...
- Snapshot date: 2026-07-16 / 快照日期：2026-07-16
...
This document records the display behavior that is implemented today for Code Mode Operations and every currently allowlisted Nested Tool Projection. It also gives one concrete fine-tuning proposal and acceptance focus for each tool so a reviewer can decide changes tool by tool. It is a display catalog, not a new canonical event contract. / 本文档记录当前已经实现的 Code Mode Operation 显示行为，以及 allowlist 中每一种嵌套工具投影的具体呈现方式；同时为每个工具给出一份具体的调优建议和验收重点，便于逐项决策。它是显示目录，不是新的 canonical event contract。
...
One grouped operation remains one canonical Logical Event with `kind: other_tool_call`, `subtype: code_mode_operation`, and `toolName: exec`. Declared nested projections do not receive event IDs, metrics, search owners, Raw refs, status, severity, or escalation tags. Independently persisted Observed Nested Activity remains visible as its own Logical Event. / 一个分组后的 operation 仍是单个 canonical 逻辑事件，使用 `kind: other_tool_call`、`subtype: code_mode_operation` 与 `toolName: exec`。声明式嵌套投影不拥有 event ID、指标、搜索 owner、Raw refs、status、severity 或提权标签。独立持久化的已观测嵌套活动仍以自己的逻辑事件显示。
...
one Code Mode Operation Logical Event
  Timeline
    ...
    - multiple safe declarations: Nested Tool Projection cards + folded source
    - uncertain program: raw JavaScript Command
    - unassociated outer output remains explicit
  Inspector
    - Operation metadata
...
一个 Code Mode Operation 逻辑事件
  时间线
    ...
    - 多个安全声明：嵌套工具投影卡片及折叠源码
    - 不确定程序：raw JavaScript Command
...
- If the whole JavaScript program passes projection eligibility with exactly one declaration, the projection wrapper is removed and its existing tool-specific sections become the timeline body. The header uses the tool's presentation label/name plus Code Mode and evidence labels; canonical event fields remain unchanged. / 如果整段 JavaScript 通过投影资格检查且恰好包含一个声明，投影 wrapper 会被移除，其既有工具专用 sections 成为时间线正文。Header 使用该工具的 presentation label/name，并显示代码模式与证据标签；canonical event 字段保持不变。
- If eligibility produces two or more declarations, cards appear in source order, followed by a collapsed `Code Mode source`; the header says `Multi-tool Code Mode operation / 多工具代码模式操作` and shows the declared-request count. / 如果资格检查产生两个及以上声明，卡片按源码顺序出现，之后是默认折叠的 `Code Mode source`；header 显示 `Multi-tool Code Mode operation / 多工具代码模式操作` 与声明 request 数量。
- If eligibility fails anywhere, no partial cards are shown. The operation falls back to a normal JavaScript `Command` section. / 只要任一位置未通过资格检查，就不显示局部卡片；整个 operation 回退为普通 JavaScript `Command` section。
...
- If a single declaration has no result association, the last observed exec/wait output remains primary under `Unassociated operation output / 未关联的操作输出`; multi-tool and raw fallback keep `Final output / 最终输出`. / 如果单个声明没有结果关联，最后一次观测到的 exec/wait 输出继续以 `Unassociated operation output / 未关联的操作输出` 作为主要内容；多工具与 raw fallback 继续使用 `Final output / 最终输出`。
...
- Let each tool choose a compact request/result layout. A single-tool header keeps Code Mode but omits the machine tool-name chip because the native event title already names the tool; request evidence, machine tool name, protocol channel, and Raw-ref count stay in the inspector. `Inferred result` appears only when a bounded result is actually displayed; `Unassociated output` appears when outer output remains separate. A multi-tool title already identifies Code Mode, so its header keeps only the tool count plus any applicable inferred-result warning; each projection still carries its card-level evidence. / 允许每种工具选择紧凑的 request/result 布局。单工具 header 保留代码模式，但由于原生事件标题已经命名工具，因此省略机器工具名 chip；request evidence、机器工具名、协议 channel 与 Raw-ref 数量留在 inspector。只有实际展示 bounded 结果时才显示“推断结果”；outer output 保持分离时显示“未关联输出”。多工具标题已经标识 Code Mode，因此其 header 只保留工具数量及适用的推断结果警示；每个投影仍保留卡片级证据。
...
- The producer currently emits only `resultAssociation: bounded` or `none`. Inspector evidence retains those machine values; the compact timeline header renders them as `Inferred result / 推断结果` only when the bounded result is visible, or `Unassociated output / 未关联输出` when outer output remains separate. / 当前 producer 只会发出 `resultAssociation: bounded` 或 `none`。Inspector 证据继续保留这些机器值；紧凑时间线 header 只有在 bounded 结果可见时才显示 `Inferred result / 推断结果`，outer output 保持分离时则显示 `Unassociated output / 未关联输出`。
