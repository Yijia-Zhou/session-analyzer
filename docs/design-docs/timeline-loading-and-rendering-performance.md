...
> Lifecycle note / 生命周期说明：The accepted Indexed/Materialized Session design is specified in `docs/design-docs/indexed-materialized-session-lifecycle.md`. M3 keeps project-wide queries resident, and the M4 candidate makes Codex Session-scoped timeline/detail loading an explicit uncached source-backed materialization operation. Per-revision cache/coalescing/scheduling remains deliberately deferred to M6. / 已接受的 Indexed／Materialized Session 设计见 `docs/design-docs/indexed-materialized-session-lifecycle.md`。M3 让项目级查询保持常驻，M4 候选把 Codex Session 范围的时间线／详情加载变为显式、无 cache 的来源回读物化操作；按 revision 的 cache／合并／调度仍有意推迟到 M6。
...
The historical baseline problem was primarily in the shared timeline loading and rendering path rather than in network latency or repeated JSONL parsing. Search is the strongest amplifier because it can load many pages, expand hit owners, rebuild highlights, and navigate into detail, but the same timeline state and renderer are also used by ordinary paging, folding, project-result drill-down, focus restoration, and Inspector navigation. M4 adds a deliberate temporary server-side amplifier for Codex: until M6, each independent event/detail request performs an uncached accepted-prefix materialization. / 历史基线问题主要位于共享的时间线加载与渲染路径，而不是网络延迟或重复解析 JSONL。搜索之所以成为最强放大器，是因为它可以加载许多分页、展开命中 owner、重建高亮并导航到详情；但普通分页、折叠、项目结果下钻、焦点恢复和检查器导航也共用同一份时间线状态与 renderer。M4 为 Codex 增加了一个有意的临时 server-side 放大项：在 M6 前，每个独立 event／detail request 都会执行一次无 cache 的 accepted-prefix 物化。
...
Project selection / 项目选择
  -> buildIndex: JSONL -> transient parsed Raw -> logicalEvents + compact Raw + presentationIndexes
       (compact projections retained in memory; source rows hydrate detail on demand)
...
`buildIndex` pre-scans transcript metadata to select repository candidates and parses each selected JSONL file as a stream. For Codex, complete parsed source records live only while one Session is being parsed and its Logical Events are being constructed. Before the next Session starts, the adapter replaces them with new Raw objects built from an explicit compact whitelist; the committed index retains only fields required by timeline, Raw Layer, search, analysis, fork inference, image descriptors, presentation, and source hydration. Logical Events and bounded presentation indexes are derived projections and do not retain aliases to the parsed source graph. Search and timeline requests reuse the compact index. Structured detail, validated Raw readback, and image preview re-read only the indexed source rows they need. Cancelling a new project-index job can stop index construction, while an already committed index remains available. Claude's resident representation is unchanged. / `buildIndex` 会预扫描转录 metadata 以选择仓库候选文件，并以流式方式解析每个选中的 JSONL 文件。对 Codex 而言，完整 parsed 来源记录只在单个 Session 的解析与 Logical Event 构建期间存在；开始下一个 Session 前，adapter 会用显式紧凑白名单构造的新 Raw 对象替换它们。已提交索引只保留 timeline、Raw Layer、搜索、分析、fork inference、图片 descriptor、presentation 与来源 hydration 所需字段。Logical Event 与有界 presentation index 都是派生投影，不会别名引用 parsed 来源对象图。搜索与时间线请求复用紧凑索引；结构化详情、经过验证的 Raw 回读与图片预览只重读所需的已索引来源行。取消新的项目索引 job 可以停止索引构建，同时已提交的旧索引继续可用。Claude 的常驻表示保持不变。

The compact index remains a material in-memory working set, so every timeline/search cache must still be scoped to one committed index revision. The Code Mode request index is deliberately name-only and independent from canonical Logical Events: it maps existing operation IDs to safe declared tool names plus `declared_source`, never to arguments, source, AST nodes, result association, or execution evidence. A single Session whose own transient parse graph exceeds the heap remains a residual limit; this change removes corpus-wide accumulation rather than introducing a persistent database or lazy Session cache. / 紧凑索引仍是不可忽略的内存 working set，因此任何 timeline／search cache 仍必须绑定到单个已提交索引 revision。Code Mode request 索引有意只保留名称，并与 canonical Logical Event 相互独立：它只把既有 operation ID 映射到安全声明的工具名称和 `declared_source`，绝不映射到参数、源码、AST 节点、result association 或执行证据。若单个 Session 自身的瞬时解析对象图超过 heap，仍可能触及残余上限；本变更消除的是语料级累积，而不是引入持久数据库或 lazy Session cache。
