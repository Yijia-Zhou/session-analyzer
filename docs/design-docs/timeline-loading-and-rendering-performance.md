...
> Lifecycle note / 生命周期说明：The accepted Indexed/Materialized Session design is specified in `docs/design-docs/indexed-materialized-session-lifecycle.md`. M3 keeps project-wide queries resident; M4 moves Codex and M5 moves Claude Session-scoped timeline/detail loading to explicit uncached source-backed materialization. Per-revision cache/coalescing/scheduling remains deliberately deferred to M6. / 已接受的 Indexed／Materialized Session 设计见 `docs/design-docs/indexed-materialized-session-lifecycle.md`。M3 让项目级查询保持常驻；M4 把 Codex、M5 把 Claude Session 范围的时间线／详情加载迁移为显式、无 cache 的来源回读物化；按 revision 的 cache／合并／调度仍有意推迟到 M6。
...
The historical baseline problem was primarily in the shared timeline loading and rendering path rather than in network latency or repeated JSONL parsing. Search is the strongest amplifier because it can load many pages, expand hit owners, rebuild highlights, and navigate into detail, but the same timeline state and renderer are also used by ordinary paging, folding, project-result drill-down, focus restoration, and Inspector navigation. M4 for Codex and M5 for Claude add a deliberate temporary server-side amplifier: until M6, each independent Session event/detail request performs an uncached accepted-prefix materialization. / 历史基线问题主要位于共享的时间线加载与渲染路径，而不是网络延迟或重复解析 JSONL。搜索之所以成为最强放大器，是因为它可以加载许多分页、展开命中 owner、重建高亮并导航到详情；但普通分页、折叠、项目结果下钻、焦点恢复和检查器导航也共用同一份时间线状态与 renderer。M4 对 Codex、M5 对 Claude 增加了一个有意的临时 server-side 放大项：在 M6 前，每个独立 Session event／detail request 都会执行一次无 cache 的 accepted-prefix 物化。
...
Project selection / 项目选择
  -> buildIndex: JSONL -> transient complete Session -> strict Indexed metadata
       + packed ProjectQueryStore + closed source dependencies
  -> /timeline: accepted-prefix materialization -> choose Layer -> structural scan
       -> text counts -> slice -> DTO page
...
`buildIndex` pre-scans transcript metadata to select repository candidates and parses accepted JSONL prefixes as streams. Codex and Claude both use a two-pass source-backed build: each complete Raw/Logical/analysis/presentation graph is transient, relationship evidence is reduced to source-owned scalar and bounded facts, and the final Index commits only strict Session metadata, packed project-query rows, catalogs, dependencies, and bounded legacy ownership. No complete event array or parsed source graph remains reachable from the committed Index. Project-wide list/search/filter requests reuse the packed query store. Session-scoped timeline, event, detail, analysis, and Raw-reference flows materialize only the selected accepted prefix under the captured index-revision owner. The M6 owner reuses one validated complete Session for the rest of that revision, coalesces concurrent same-Session requests, and admits distinct cold Sessions through one process-local active slot plus a 32-job per-owner FIFO queue. Cancelling a new project-index job can stop index construction while an already committed Index and its cache remain available. / `buildIndex` 会预扫描 transcript metadata 以选择 repository candidate，并以流式方式解析已接受的 JSONL prefix。Codex 与 Claude 都使用双 pass 来源回读构建：每个完整 Raw／Logical／analysis／presentation graph 都只瞬态存在，关系证据会缩减为来源拥有的 scalar 与有界 fact，最终 Index 只提交严格 Session metadata、紧凑 project-query row、catalog、dependency 与有界 legacy ownership。已提交 Index 中不存在可达的完整 event array 或 parsed 来源图。项目级 list／search／filter 请求复用紧凑 query store；Session 范围的 timeline、event、detail、analysis 与 Raw-reference flow 只在捕获的 index-revision owner 下物化所选已接受 prefix。M6 owner 会在该 revision 的剩余生命周期内复用一份已校验完整 Session、合并同 Session 并发请求，并通过一个进程内 active slot 加每 owner 32-job FIFO queue 接纳不同 cold Session。取消新的 project-index job 可以停止 index construction，同时已提交的旧 Index 及其 cache 继续可用。

The strict Index and packed query store remain a material in-memory working set, so every future materialization cache and every query response must stay scoped to one committed index revision. Codex's Code Mode request index is deliberately name-only and independent from canonical Logical Events: it maps existing operation IDs to safe declared tool names plus `declared_source`, never to arguments, source, AST nodes, result association, or execution evidence. A single Session whose own transient parse graph exceeds the heap remains a residual limit; this change removes corpus-wide complete-event accumulation rather than introducing a persistent database or lazy Session cache. / 严格 Index 与紧凑 query store 仍是不可忽略的内存 working set，因此未来任何物化 cache 与每个 query response 都必须绑定到一个已提交 index revision。Codex 的 Code Mode request index 有意只保留名称，并与 canonical Logical Event 相互独立：它只把既有 operation ID 映射到安全声明的工具名称和 `declared_source`，绝不映射到参数、源码、AST node、result association 或 execution evidence。若单个 Session 自身的瞬时 parse graph 超过 heap，仍可能触及残余上限；本变更消除的是语料级完整 event 累积，而不是引入持久 database 或 lazy Session cache。

The final pre-M5-re-review Claude profile used the adapter-default home visible for this repository and found zero Claude Sessions, so it proves only that the strict path completes under the default heap on the actually available empty corpus. It recorded 0／0／0 Main／Protocol／Raw rows, a 3.4 ms build, 10,216,208-byte observed peak heap, 62,656,512-byte process maximum RSS, and 9,529,144-byte forced-GC heap. These numbers are not evidence for non-empty Claude scale; deterministic source-semantic and fork-heavy retained-shape fixtures are the executable M5 gate until a representative non-empty aggregate corpus is available. / 最终 M5 复审前的 Claude profile 使用本仓库可见的 adapter-default home，但未发现 Claude Session，因此它只证明严格路径能在实际可用的空语料上以默认 heap 完成。该运行记录 0／0／0 条 Main／Protocol／Raw row、3.4 ms build、10,216,208-byte observed peak heap、62,656,512-byte process maximum RSS 与 9,529,144-byte forced-GC heap。这些数字不能证明非空 Claude 规模；在获得有代表性的非空聚合语料前，确定性来源语义与 fork-heavy retained-shape fixture 是可执行的 M5 门槛。

### Final Indexed/Materialized lifecycle measurement / 最终 Indexed／Materialized 生命周期量测

The final M7 aggregate-only Codex run used the default 2,348,810,240-byte V8 heap and the adapter-default source visible for this repository. The evolved corpus contained 488 Sessions and 74,037／141,830／300,997 Main／Protocol／Raw rows. The cold strict build completed in 98,982.0 ms; observed transient peak heap／sampled RSS were 484,297,448／1,883,508,736 bytes, process maximum RSS was 1,900,810,240 bytes, and forced-GC committed heap／RSS／external／ArrayBuffer were 55,740,944／1,768,493,056／1,046,905,696／1,044,581,493 bytes. The packed query store／recorded Index total accounted for 1,044,074,376／1,063,004,643 bytes. Packed Main／Protocol／Raw median scans were 538.59／682.99／1,599.91 ms. / 最终 M7 只输出聚合数据的 Codex 运行使用默认 2,348,810,240-byte V8 heap 与本仓库可见的 adapter-default 来源。演进语料包含 488 个 Session 与 74,037／141,830／300,997 条 Main／Protocol／Raw row。严格 cold build 以 98,982.0 ms 完成；观测 transient peak heap／sampled RSS 为 484,297,448／1,883,508,736 bytes，process maximum RSS 为 1,900,810,240 bytes，强制 GC 后 committed heap／RSS／external／ArrayBuffer 为 55,740,944／1,768,493,056／1,046,905,696／1,044,581,493 bytes。紧凑 query store／记录的 Index total accounted 1,044,074,376／1,063,004,643 bytes。紧凑 Main／Protocol／Raw scan median 为 538.59／682.99／1,599.91 ms。

M0's evolving 482-Session diagnostic build contained 200,820 Logical and 282,998 Raw rows, required a 4 GiB diagnostic heap to complete, and retained 2,098,809,064-byte forced-GC heap plus 2,674,880,512-byte RSS. The final corpus has 215,867 Logical rows (+7.5%) and 300,997 Raw rows (+6.4%), while forced-GC heap is directionally 97.3% lower and RSS 33.9% lower. This is not a same-snapshot causal benchmark: the corpus changed, and about 1.04 GB now deliberately lives in packed external/ArrayBuffer storage rather than V8 heap. The exact reliability result is that the larger final corpus completes without a heap override. / M0 演进中的 482 Session 诊断 build 包含 200,820 条 Logical 与 282,998 条 Raw row，需要 4 GiB 诊断 heap 才能完成，并在强制 GC 后保留 2,098,809,064-byte heap 与 2,674,880,512-byte RSS。最终语料包含 215,867 条 Logical（+7.5%）与 300,997 条 Raw（+6.4%），而强制 GC 后 heap 方向性降低 97.3%，RSS 降低 33.9%。这不是同一 snapshot 的因果 benchmark：语料已经变化，且约 1.04 GB 现在有意位于紧凑 external／ArrayBuffer storage，而不是 V8 heap。精确的可靠性结果是：更大的最终语料无需 heap override 即可完成。

A separate content-free cache profile selected the largest accepted-bytes Session: 116,936,717 source bytes, 8,749 Raw rows, and 6,351 Logical rows. Cold materialization took 6,545.3 ms; an exact-identity warm hit took 0.263 ms with one adapter call total. The one-entry cache added 44,031,640 heap bytes, 15,563,350 external bytes, and 9,101,312 RSS bytes after GC relative to the committed Index in that process. Retirement cleared the cache owner and following-GC RSS returned to within 360,448 bytes of its pre-cache baseline; heap remained 35,429,024 bytes higher, so GC state is not presented as exact object-reclamation accounting. This measures one large Session, not the unbounded all-cache case; no-eviction remains tracked debt. / 独立的不含内容 cache profile 选择了 accepted bytes 最大的 Session：116,936,717 source bytes、8,749 条 Raw row 与 6,351 条 Logical row。冷物化耗时 6,545.3 ms；精确 identity warm hit 耗时 0.263 ms，总计只调用一次 adapter。相对同一进程中已提交 Index，单 entry cache 在 GC 后增加 44,031,640 heap bytes、15,563,350 external bytes 与 9,101,312 RSS bytes。Retirement 清除 cache owner，随后 GC 后 RSS 回到 cache 前基线的 360,448 bytes 范围内；heap 仍高 35,429,024 bytes，因此不会把 GC 状态表述为精确 object 回收 accounting。该结果量测一个大 Session，而不是无界的全 cache case；no-eviction 继续作为已跟踪 debt。

The final adapter-default Claude run still found zero Sessions. It completed in 3.1 ms with 10,187,896-byte observed peak heap, 62,771,200-byte process maximum RSS, and 9,376,024-byte forced-GC heap. This supersedes the earlier empty-run numbers but still makes no non-empty Claude scale claim; deterministic source-semantic and fork-heavy retained-shape fixtures remain the executable evidence. / 最终 adapter-default Claude 运行仍发现 0 个 Session。它以 3.1 ms 完成，观测 peak heap 为 10,187,896 bytes，process maximum RSS 为 62,771,200 bytes，强制 GC 后 heap 为 9,376,024 bytes。该结果取代先前的空运行数字，但仍不声称非空 Claude 规模；确定性来源语义与 fork-heavy retained-shape fixture 继续作为可执行证据。

### Measured Codex resident-index decision / 已测 Codex 常驻索引决策

On 2026-08-12, aggregate-only profiling on one sensitive 0.84 GB Codex corpus confirmed the lifecycle problem and the compact boundary. Before compaction, the default 2.349 GB V8 heap failed with fatal OOM around 31 seconds; a 6 GiB diagnostic completion measured 2.59 GB peak and 2.45 GB post-GC `heapUsed`. Deleting only `raw.parsed` in an uncommitted experiment left 1.97 GB post-GC, while rebuilding Raw from a conservative explicit whitelist estimated 1.61 GB. The implemented per-Session compact index then completed the evolving 0.85 GB corpus under the default heap: 452 Sessions, 242,902 Raw Records, 167,627 Logical Events, 38.0 seconds cold, 1.874 GB peak and 1.708 GB post-GC `heapUsed`, and zero resident parsed Raw Records. This is about 20% peak and 27% post-GC heap headroom. / 2026-08-12，仅保留聚合数据的敏感语料 profiling 在一个约 0.84 GB 的 Codex corpus 上确认了生命周期问题与紧凑边界。紧凑化前，默认 2.349 GB V8 heap 约 31 秒即 fatal OOM；使用 6 GiB 完成的诊断运行测得 peak／post-GC `heapUsed` 为 2.59／2.45 GB。未提交实验中只删除 `raw.parsed` 后，post-GC 仍为 1.97 GB；从保守显式白名单重建 Raw 则估算为 1.61 GB。实现按 Session 紧凑索引后，持续增长到约 0.85 GB 的语料在默认 heap 下完成：452 个 Session、242,902 条 Raw Record、167,627 个 Logical Event，cold 38.0 秒，peak／post-GC `heapUsed` 为 1.874／1.708 GB，resident parsed Raw Record 为零；对应约 20% peak 与 27% post-GC heap 余量。

A same-window aggregate comparison then measured 453 Sessions and about 245k Raw Records while the live corpus grew by only 67 Raw Records between runs. The pre-change implementation required a 6 GiB limit to complete: cold peak／post-GC RSS was 3.053／2.969 GB and `heapUsed` was 2.817／2.546 GB. The compact implementation completed under the default heap with peak／post-GC RSS of 2.209／2.151 GB and `heapUsed` of 1.873／1.724 GB. That is about 27.6% lower RSS and 33.5%／32.3% lower peak／post-GC heap on nearly the same evolving corpus. The heap-limit difference is stated explicitly: this comparison establishes process-memory direction and magnitude, while the default-heap OOM versus successful completion remains the reliability result. / 随后的同一时间窗聚合对照测量了 453 个 Session 与约 24.5 万条 Raw Record；两次运行间活跃语料只增长了 67 条 Raw Record。变更前实现需要 6 GiB limit 才能完成：cold peak／post-GC RSS 为 3.053／2.969 GB，`heapUsed` 为 2.817／2.546 GB。紧凑实现则在默认 heap 下完成，peak／post-GC RSS 为 2.209／2.151 GB，`heapUsed` 为 1.873／1.724 GB。对近似相同且仍在增长的语料，RSS 约下降 27.6%，peak／post-GC heap 分别约下降 33.5%／32.3%。这里明确说明 heap limit 不同：该对照用于确认 process memory 的方向与量级；默认 heap 从 OOM 变为成功完成仍是运行可靠性结论。

After final retained-shape review hardening, the corpus had grown to 904.4 MB, 454 Sessions, 248,997 Raw Records, and 172,559 Logical Events. The final default-heap cold build completed in 33.6 seconds with 2.198／2.143 GB peak／post-GC RSS and 1.925／1.762 GB `heapUsed`; resident parsed Raw Records remained zero. Two true no-change runs reused 454/454 Sessions in about 5.4 seconds each with roughly 1.94／1.77 GB peak／post-GC `heapUsed`. The larger snapshot leaves about 423 MB (18%) peak and 587 MB (25%) post-GC heap headroom. This remains material headroom, but further corpus growth toward that margin is the explicit evidence trigger for a separate M5 lightweight-index／lazy-Session design review. / 最终 retained-shape 审查加固后，语料已增长到 904.4 MB、454 个 Session、248,997 条 Raw Record 与 172,559 个 Logical Event。最终默认 heap cold build 在 33.6 秒内完成，peak／post-GC RSS 为 2.198／2.143 GB，`heapUsed` 为 1.925／1.762 GB；resident parsed Raw Record 仍为零。两次真正 no-change 均约 5.4 秒并复用 454/454 个 Session，peak／post-GC `heapUsed` 约为 1.94／1.77 GB。该更大快照仍保留约 423 MB（18%）peak 与 587 MB（25%）post-GC heap headroom；这是有实质意义的余量，但语料继续增长并逼近该余量，就是另行评审 M5 lightweight-index／lazy-Session 设计的显式证据触发条件。

Exact reuse fingerprints add sequential I/O only after size／mtime and fork-digest eligibility checks. On the live large corpus, immediately following changed-session reindexes completed in 5.5 seconds with 451/452 reused and 7.9 seconds with 450/452 reused. Because active transcripts grew, these are recorded as changed rather than no-change. On a separate inactive 251.4 MB corpus, cold build took 15.6 seconds and two reproducible no-change runs took about 5.9 seconds each with 19/19 Sessions reused; peak／post-GC `heapUsed` was about 0.733／0.576 GB. These measurements justify the exact fingerprint cost for snapshot correctness. They do not justify removing search text, redesigning search, adopting a larger default heap, or moving to a lightweight-index/lazy-Session architecture in this milestone. No transcript, local path, prompt, heap snapshot, or raw diagnostic dump is committed. / 精确 reuse 指纹只会在大小／mtime 与 fork digest eligibility 初筛后增加顺序 I/O。在活跃大型语料上，紧随其后的 changed-session reindex 分别以 451/452 复用在 5.5 秒完成、以 450/452 复用在 7.9 秒完成；由于活跃 transcript 发生增长，这些数据被记录为 changed，而不是 no-change。另一个不活跃的 251.4 MB 语料中，cold build 耗时 15.6 秒，两次可复现 no-change 均约 5.9 秒并复用 19/19 个 Session；peak／post-GC `heapUsed` 约为 0.733／0.576 GB。这些测量支持为快照正确性支付精确指纹成本，但不支持删除 search text、重做搜索、采用更大默认 heap，或在本里程碑转向 lightweight-index／lazy-Session 架构。未提交任何 transcript、本地路径、prompt、heap snapshot 或原始诊断 dump。

### Operational large-history guidance / 大型历史运行指引

Capacity guidance is based on matching transcript history, not source repository size. The measured 251.4 MB Codex history at about 0.733 GB peak heap and the evolved 847.7–904.4 MB histories at about 1.87–1.93 GB peak heap support a single conservative warning threshold of 800 MiB candidate bytes. This threshold sits at the lower edge of the observed high-memory region so users and agents receive guidance before parsing begins, but it is explicitly empirical: candidate bytes alone cannot predict heap because Raw and Logical counts, record composition, Node version, and a single unusually large Session also matter. Histories approaching roughly 1 GB of matching transcript data are high-memory workloads, not guaranteed failures or permanent product limits. / 容量指引以匹配的 transcript 历史为依据，而不是源码仓库大小。251.4 MB Codex 历史约 0.733 GB heap 峰值，以及演进到 847.7–904.4 MB 的历史约 1.87–1.93 GB heap 峰值，共同支持使用单一、保守的 800 MiB candidate bytes 警告阈值。该阈值位于已观测高内存区域的下沿，让用户和 agent 能在解析开始前收到指引，但它明确只是经验值：candidate bytes 本身无法预测 heap，因为 Raw 与 Logical 数量、记录组成、Node 版本和单个异常庞大的 Session 也会产生影响。接近约 1 GB 匹配 transcript 数据的历史属于高内存工作负载，不代表必然失败，也不是永久的产品上限。

The source-neutral server job boundary observes the already-produced transition into `phase: parsing`; it does not rescan the corpus. At most once per indexing operation it emits `[SESSION_ANALYZER_LARGE_TRANSCRIPT_HISTORY]`, records the same signal in opt-in diagnostics, and then leaves indexing, cancellation, exit behavior, `NODE_OPTIONS`, and the V8 heap unchanged. The message directs humans and agents to attempt normal indexing first and to use a moderately larger temporary heap only after authoritative `JavaScript heap out of memory` evidence. Diagnostic progress records expose current plus process-local peak memory; the capacity record contains only its stable code, empirical threshold, and aggregate candidate file/byte counts. Existing allowlists continue to exclude paths and content. A fatal OOM can prevent terminal logger flush, so stderr remains the final crash evidence. Memory efficiency for large histories remains active improvement work, and further corpus growth toward default-heap headroom or one individually enormous Session remains the evidence trigger for a separate scale design review. / 来源中立的 server job 边界观察索引器已经产生的 `phase: parsing` 转换，不会重新扫描语料。每次索引操作最多输出一次 `[SESSION_ANALYZER_LARGE_TRANSCRIPT_HISTORY]`，在选择性启用的诊断中记录同一信号，然后保持索引、取消、退出行为、`NODE_OPTIONS` 与 V8 heap 不变。消息要求用户与 agent 先尝试普通索引，并且只有在权威的 `JavaScript heap out of memory` 证据出现后才使用适度增大的临时 heap。诊断进度记录会暴露当前与进程内峰值内存；容量记录只包含稳定代码、经验阈值和聚合 candidate 文件／字节数。现有 allowlist 继续排除路径与内容。Fatal OOM 可能使 logger 无法刷新终态，因此 stderr 仍是最终崩溃证据。大型历史的内存效率仍是持续改进工作；语料继续增长并逼近默认 heap 余量，或出现单个异常庞大的 Session，仍是另行开展规模设计评审的证据触发条件。
...
`GET /api/sessions/:sessionId/timeline` accepts `offset`, `limit`, `layer`, `q`, `kind`, `status`, `tool`, `file`, Main-only `codeModeRequest`, and `locale`. After the selected strict Indexed Session is materialized from its accepted prefix, `sourceEventsForLayer` selects request-scoped Logical Events for Main and Protocol or maps the request-scoped physical Raw Records for Raw Layer. `getTimeline` then: / `GET /api/sessions/:sessionId/timeline` 接收 `offset`、`limit`、`layer`、`q`、`kind`、`status`、`tool`、`file`、仅 Main 的 `codeModeRequest` 与 `locale`。所选严格 Indexed Session 从其已接受 prefix 物化后，`sourceEventsForLayer` 会为 Main／Protocol 选择请求范围的 Logical Event，或为 Raw Layer 映射请求范围的物理 Raw Record。随后 `getTimeline` 会：
...
The page size bounds response size and page DTO mapping, but it does not bound accepted-prefix materialization, structural filtering, text counting, Raw Layer DTO creation, event-kind catalog work, or Main-layer request-catalog work. The request catalog reads the request-scoped Materialized presentation index and counts operations; project-wide request filters continue to use the resident packed query store. / Page size 会限制响应体与分页 DTO mapping，但不会限制 accepted-prefix 物化、结构筛选、文本计数、Raw Layer DTO 创建、event-kind catalog 工作或 Main 层 request catalog 工作。Request catalog 读取请求范围的 Materialized presentation index 并按 operation 计数；项目级 request filter 继续使用常驻的紧凑 query store。

For a non-empty logical response, public DTO composition may build a request-scoped reverse map for Code Mode presentation context. It performs one linear pass over the selected Materialized Session's logical events and their existing `eventRefs`, records only uniquely proven `code_mode_operation` parents, and attaches an optional context only to matching DTOs on the requested page or layer-aware event envelope. The transient map is not retained in the canonical Index, does not run for Raw DTOs, and creates no additional timeline membership, target, count, or pagination work. Its cost is intentionally an explicit `O(E + R)` response-projection boundary after materialization, where `E` is Session logical-event count and `R` is the total examined `eventRefs` entries.

对于非空的逻辑响应，公开 DTO 组成可以为 Code Mode 呈现上下文构建一个请求范围的反向映射。它会对所选 Materialized Session 的逻辑事件及其既有 `eventRefs` 做一次线性遍历，只记录唯一已证明的 `code_mode_operation` 父操作，并且只把可选上下文附加到请求页面或感知事件层 event envelope 中匹配的 DTO。该临时映射不会保留在规范 Index 中，不会用于 Raw DTO，也不会产生额外时间线成员、目标、计数或分页工作。其成本是在物化后有意保留的显式 `O(E + R)` 响应投影边界，其中 `E` 是 Session logical-event count，`R` 是检查过的 `eventRefs` 条目总数。

#### 3. Detail request lifetime / 详情请求生命周期

Timeline DTOs contain stable identity, presentation metadata, previews, hit state, and traceability fields but not the full structured body. Expanded or selected cards request `GET /api/sessions/:sessionId/events/:eventId/detail?layer=...`. Under the same captured revision, the server materializes the selected Session, finds the request-scoped event, resolves its supporting Raw Records, builds timeline and Inspector sections, sanitizes the result, and returns a detail DTO. / 时间线 DTO 包含稳定 identity、展示 metadata、preview、命中状态与可追溯字段，但不包含完整结构化正文。展开或选中的卡片会请求 `GET /api/sessions/:sessionId/events/:eventId/detail?layer=...`。在同一个已捕获 revision 下，服务端会物化所选 Session、查找请求范围的 event、解析其依据的 Raw Record、构建 timeline 与 Inspector section、清洗结果并返回 detail DTO。
...
Raw References use a separate evidence path: the browser normally requests each referenced source row through the indexed `/api/sessions/:sessionId/raw/:rawId` route rather than reusing the structured detail DTO. The temporary legacy Codex `/api/raw?file&line` route also resolves the requested locator back to exactly one indexed Raw Record and applies the same row-digest freshness check; it no longer performs a free-form source read. Event-reference navigation first resolves an event envelope through `/events/:eventId`, which is Layer-aware but independent of the active timeline filters, and may temporarily reveal that event without changing filtered membership. / 原始引用使用独立的证据路径：浏览器通常通过已索引的 `/api/sessions/:sessionId/raw/:rawId` route 请求每条来源记录，而不是复用结构化详情 DTO。临时保留的旧 Codex `/api/raw?file&line` route 也会把所请求 locator 反向解析到唯一一条已索引 Raw Record，并应用相同的逐行 digest freshness 校验；它不再执行自由形式的来源读取。事件引用导航会先通过 `/events/:eventId` 解析事件 envelope；该端点感知事件层但独立于当前时间线筛选，并且可以临时展示该事件而不改变筛选成员关系。
...
M3 adds a separate Index-revision boundary beneath these request owners. Every successful Index install or identity clear retires the captured `IndexRevisionLease`; asynchronous project scans and every materializing route validate that same lease after await before sending a response. Revision-bearing Session lists, project-result lists, and file-suggestion responses carry additive `indexRevision` and are checked before browser commit. A mismatch or server 409 `INDEX_REVISION_RETIRED` discards the response and stale navigation state, coalesces one state/session/query reload, and never exposes a stale ordinal to timeline drill-down. Nested mismatches propagate failure into the active recovery retry loop; a mismatch at the loop's completion boundary queues one successor recovery, so coalescing remains convergent across overlapping replacements. This is revision safety only: materialization cache, coalescing, and scheduling remain deferred to M6. / M3 在这些 request owner 下方增加独立的 Index revision 边界。每次成功 Index install 或 identity clear 都会 retire 捕获的 `IndexRevisionLease`；异步项目扫描与每个物化 route 都会在 await 后、发送 response 前校验同一 lease。带 revision 的 Session 列表、项目结果列表与文件建议 response 会携带 additive `indexRevision`，并在 browser commit 前校验。Mismatch 或 server 409 `INDEX_REVISION_RETIRED` 会丢弃 response 与过期导航状态、合并一次 state／Session／query reload，绝不向 timeline 下钻暴露过期 ordinal。嵌套 mismatch 会把失败传播给当前恢复重试循环；在循环完成边界检测到的 mismatch 会排队一次后继恢复，使合并在重叠 replacement 下仍可收敛。这只提供 revision safety；materialization cache、coalescing 与 scheduling 继续推迟到 M6。

The accepted structured-filter viewport checkpoint is deterministic: preserve the exact selected event only when it exists in the newly committed first page; otherwise close that selection and reset to top. Layer and `Read from here` restoration remain explicit deep-prefix operations. Ordinary bottom pagination cannot run until the replacement epoch commits and then receives a new qualifying user-input token. The timeline pane is keyboard-focusable, and non-interactive pointer activation moves focus into it so scrolling keys reach the pane-owned intent listener; interactive descendants retain their own focus and remain excluded from pointer/keyboard scroll capture, so Load more activation cannot authorize a duplicate append. For the pane-owned, unmodified `End` key, the listener prevents the platform-specific native default and explicitly moves the pane to its bottom only after authorizing the one-shot keyboard intent; the resulting positive scroll remains the evidence that claims pagination. / 已接受的结构化筛选视口 checkpoint 是确定性的：只有精确所选事件存在于新提交的第一页时才保留，否则关闭该选择并回到顶部。事件层与“从此处阅读”恢复继续属于显式深前缀操作。普通底部分页必须等 replacement epoch 提交并收到新的合格用户输入 token 后才能运行。时间线面板可通过键盘聚焦，非交互指针激活会把焦点移入面板，使滚动键能够到达面板自己的意图监听器；交互式后代继续保留自身焦点，并从 pointer/keyboard scroll 捕获中排除，因此“加载更多”激活不能授权重复追加。对于由面板拥有且未带修饰键的 `End` 按键，监听器会阻止各平台不同的原生默认行为，并只在授权一次性键盘意图后显式把面板移动到底部；随后产生的正向滚动仍是领取分页权限所需的证据。
...
- The M4 browser migration demonstrates why M6 cache/coalescing is a functional performance boundary rather than an optional micro-optimization. A 171-row project drill-down correctly loaded and selected the latest match, but the default expanded profile then issued 171 detail requests; uncached Codex reconstruction stretched the focused case to minutes and left old-context work visible to a later filter transition. M4 keeps the assertions and records the full-suite result (114/116); it does not weaken browser semantics or implement the M6 owner early. / M4 browser 迁移证明 M6 cache／合并是功能性性能边界，而非可选微优化。一个 171-row 项目下钻正确加载并选中最新命中，但默认展开 profile 随后发出 171 个 detail request；无 cache 的 Codex 重建把聚焦用例拉长到数分钟，并让旧 context work 延续到后续 filter transition。M4 保留这些断言并记录全套结果（114／116）；不会弱化 browser 语义，也不会提前实现 M6 owner。
...
- 2026-08-10: Accepted bounded Codex fork-digest scope after read-only paired measurement. The optimized cold index computes pre-externalization canonical JSON plus SHA-256 only for potential materialized-fork children and their unique retained parents. Four balanced live-corpus pairs measured 22.545 s digest-on versus 22.030 s sentinel-off: +2.34% mean wall time and +2.23% median, below the 5% gate, versus +17.17% before bounding. Process CPU was +6.66%, while peak RSS and retained heap showed no increase (-0.68% and -0.018%). No-fork sanitized fixtures retained zero digest arrays; the large-image fixture measured -0.36% wall time and the long-text fixture -0.87%. The live corpus changed slightly during measurement, so future release profiling should prefer an immutable snapshot, but the paired result and fixed no-fork fixtures support the bounded candidate design. / 2026-08-10：只读配对测量接受 Codex fork digest 的有界范围。优化后的冷索引只为潜在 materialized-fork child 及其唯一、已保留 parent 计算 externalization 之前的 canonical JSON 与 SHA-256。真实语料的四组平衡配对测量为 digest-on 22.545 秒、sentinel-off 22.030 秒：wall 平均回归 +2.34%、中位数 +2.23%，低于 5% 门槛；有界化之前为 +17.17%。Process CPU 回归 +6.66%，peak RSS 与 retained heap 没有增长（分别为 -0.68% 与 -0.018%）。无 fork 脱敏 fixture 保持零 digest array；大图片 fixture 的 wall 为 -0.36%，长文本 fixture 为 -0.87%。真实语料在测量期间有轻微变化，因此未来 release profiling 应优先使用不可变快照，但配对结果与固定无 fork fixture 均支持当前有界候选设计。
- 2026-08-13: Accepted per-committed-index Codex hydration coordination after deterministic cancellation tests and an aggregate-only default-heap live-corpus check. The evolved corpus contained 456 Sessions, 251,995 Raw Records, 175,036 Logical Events, and about 882 MiB candidate bytes. Cold indexing completed in 33.1 s with 1,909／1,702 MiB peak／post-GC heap; a following changed-session reindex reused 455／456 Sessions in 6.8 s with 1,914／1,702 MiB peak／post-GC heap. Six late details from the largest Session scanned 50,045 total source lines with observed scan concurrency exactly two, completed in 575 ms, and moved heap from 1,701 MiB to a 1,744 MiB transient peak before returning to 1,702 MiB after GC. Four requests cancelled while queued performed no file open; an active request stopped at row 100 while an independent request completed. No OOM was reproduced. The bounded FIFO plus HTTP disconnect propagation removes the obvious overlap amplification without a parsed-record cache, byte-offset index, or search redesign. / 2026-08-13：在确定性取消测试与只保留聚合值的默认堆真实语料检查后，接受作用域绑定到各 committed index 的 Codex hydration coordinator。演进后的语料包含 456 个 Session、251,995 条 Raw Record、175,036 个 Logical Event 与约 882 MiB candidate bytes。Cold indexing 在 33.1 秒内完成，peak／post-GC heap 为 1,909／1,702 MiB；随后一次 changed-session reindex 在 6.8 秒内复用 455／456 个 Session，peak／post-GC heap 为 1,914／1,702 MiB。最大 Session 中 6 个靠后 detail 合计扫描 50,045 个来源行，观测到的扫描并发精确为 2，总耗时 575 ms；heap 从 1,701 MiB 短暂升至 1,744 MiB，GC 后回到 1,702 MiB。4 个在队列中取消的请求未打开文件；一个活跃请求在第 100 行停止，同时另一独立请求正常完成。未复现 OOM。有界 FIFO 加 HTTP disconnect 传播已移除明显的 overlap 放大机制，无需 parsed-record cache、byte-offset index 或 search 重构。
- 2026-08-16: The M4 Codex source-backed candidate preserves the accepted no-cache milestone sequence. Strict browser fixtures pass 114/116; the two remaining long-search/pagination failures are deterministic amplification from many independent detail/timeline requests, not identity or result-parity failures. Diagnostic inspection confirmed that the 171-row project drill-down loaded the requested `offset=0&limit=171`, selected “Long timeline row 170,” and retained correct scope/mobile state before 171 background detail requests prolonged settlement. The cache/coalescing/scheduler fix remains owned by M6. / 2026-08-16：M4 Codex 来源回读候选保持已接受的无 cache milestone 顺序。严格 browser fixture 通过 114／116；剩余两个长搜索／分页失败是大量独立 detail／timeline request 的确定性放大，不是 identity 或结果等价失败。诊断检查确认 171-row 项目下钻加载了所请求的 `offset=0&limit=171`、选中 “Long timeline row 170”，并在 171 个后台 detail request 延长 settle 前保持正确 scope／mobile state。Cache／合并／scheduler 修复继续归 M6 所有。
- 2026-08-16 correction: The preceding 114/116 entry records the initial M4 candidate and is superseded by the corrected 116/116 strict-browser result. M4-local lifecycle correctness now aborts every detail hydration on project return, checks the current owner/context around navigation page fetches, gives background navigation paging a cooperative cancellable handoff, and ignores obsolete navigation/detail completion callbacks. These repairs close transition and focus accessibility without adding cross-request reuse. M6 still owns the materialized-Session cache, request coalescing, global scheduling, and warm reuse. / 2026-08-16 修正：上一条 114／116 记录的是初始 M4 候选，现由修正后的严格 browser 116／116 结果取代。M4 本地生命周期正确性现会在返回项目时取消全部 detail hydration、在 navigation page fetch 前后检查当前 owner／context、为后台 navigation pagination 提供协作式可取消 handoff，并忽略过期的 navigation／detail completion callback。这些修复在不增加跨请求复用的情况下关闭 transition 与 focus accessibility 问题。Materialized Session cache、request 合并、全局调度与 warm reuse 仍由 M6 负责。
