...
M3 adds a separate Index-revision boundary beneath these request owners. Every successful Index install or identity clear retires the captured `IndexRevisionLease`; asynchronous project scans and every materializing route validate that same lease after await before sending a response. Project responses carry additive `indexRevision`. If a project card/ordinal belongs to a different revision, or the server returns 409 `INDEX_REVISION_RETIRED`, the browser discards the stale result, coalesces a state/session/query reload, and never issues a timeline request with the stale ordinal. This is revision safety only: materialization cache, coalescing, and scheduling remain deferred to M6. / M3 在这些 request owner 下方增加独立的 Index revision 边界。每次成功 Index install 或 identity clear 都会 retire 捕获的 `IndexRevisionLease`；异步项目扫描与每个物化 route 都会在 await 后、发送 response 前校验同一 lease。项目 response 增加 additive `indexRevision`。若项目 card／ordinal 属于不同 revision，或 server 返回 409 `INDEX_REVISION_RETIRED`，browser 会丢弃过期结果、合并一次 state／Session／query reload，并且绝不使用过期 ordinal 发出 timeline request。这只提供 revision safety；materialization cache、coalescing 与 scheduling 继续推迟到 M6。
