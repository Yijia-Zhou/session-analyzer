...
- Real-project in-app-browser acceptance used repository `G:\vibe\session-analyzer`, session `019f4c61-f335-7031-8706-d480773f7d35`, Main-layer query `patch`, and the Chinese locale. Initial evidence was **28 canonical event IDs / 696 Full-text hits / 63 timeline marks / 32 Inspector marks**. One Next action switched to an Inspector with **2 marks** and changed the active owner from `call_AItiEofqOxbPf0xCEvl035Wt` to `call_Q0MFfnQZwn1NZ9f0cax15D0H`, while the ordered 28-ID array stayed byte-for-byte identical. / 真实项目内置浏览器验收使用 repository `G:\vibe\session-analyzer`、session `019f4c61-f335-7031-8706-d480773f7d35`、Main 层 query `patch` 与中文 locale。初始证据为 **28 个 canonical 事件 ID / 696 个全文命中 / 63 个 timeline mark / 32 个 Inspector mark**。一次 Next 切换到含 **2 个 mark** 的 Inspector，活动 owner 从 `call_AItiEofqOxbPf0xCEvl035Wt` 变为 `call_Q0MFfnQZwn1NZ9f0cax15D0H`，有序 28-ID 数组逐字节保持一致。
- In the same real session, explicit `Load more` changed canonical membership from **28 to 45** by appending **17 concrete event-owner IDs** while preserving the original 28-ID prefix, active ID, selected event, detail view, timeline scroll position, and Inspector mark count; pending cleared normally. This directly rejects the original Inspector-only `5 -> 11 -> 43` inflation path. / 在同一真实 session 中，显式“加载更多”通过追加 **17 个具体事件 owner ID** 把 canonical 成员从 **28 增至 45**，同时保持原 28-ID 前缀、活动 ID、所选事件、详情视图、timeline 滚动位置与 Inspector mark 数不变；pending 正常清除。这直接排除了原始 Inspector-only `5 -> 11 -> 43` 膨胀路径。
