...
- Status: completed; all required-environment and CI gates passed / 状态：已完成；要求环境与 CI 门槛全部通过
- Started: 2026-08-16 / 开始日期：2026-08-16
- Completed: 2026-08-19 / 完成日期：2026-08-19
- Branch: `support-dsh` / 分支：`support-dsh`
- Final CI-verified accepted head: `c382c072e631ddd5dc4c86c9c5b418c3e85a6044` / 最终 CI 验证接受 head：`c382c072e631ddd5dc4c86c9c5b418c3e85a6044`
- CI: GitHub Actions run `32261493322` at `c382c072e631ddd5dc4c86c9c5b418c3e85a6044` completed success for Node 22/24 on Ubuntu, Node 24 on Windows, package smoke on Ubuntu/Windows, browser on Ubuntu, and aggregate `ci`; run `32260762462` at `89587b95fe93896455354850b7a1df9ff08e3eb7` also verified the fixture fix before archival / CI：GitHub Actions run `32261493322` 在 `c382c072e631ddd5dc4c86c9c5b418c3e85a6044` 全部成功，包括 Ubuntu Node 22/24、Windows Node 24、Ubuntu/Windows package smoke、Ubuntu browser 与聚合 `ci`；run `32260762462` 在 `89587b95fe93896455354850b7a1df9ff08e3eb7` 也已在归档前验证 fixture 修复
...
- 2026-08-17 (Browser source-cycle closeout): updated the three stale two-source chooser assertions in `e2e/browser.test.js` for the registered third source. The real-source chooser test now cycles Claude Code → DeepSeek Harness → Codex with an empty temporary `dshHome`; the mocked registry-cycle test explicitly includes DeepSeek Harness and Future Source; and the server-source test expects Claude Code → DeepSeek Harness. Targeted browser rerun for the three tests: 3/3 PASS. / 更新 `e2e/browser.test.js` 中三个旧的仅两个来源 chooser 断言以匹配已注册的第三来源。真实来源 chooser 测试现以空临时 `dshHome` 按 Claude Code → DeepSeek Harness → Codex 循环；mock registry 循环测试显式包含 DeepSeek Harness 与 Future Source；server-source 测试按 Claude Code → DeepSeek Harness 断言。三个测试定向重跑：3/3 通过。
...
- 2026-08-19 (Cross-platform fixture integrity / closeout): confirmed the Windows `npm test` failure was caused by Git checking out the physical uncompressed DSH `session.jsonl` with CRLF, adding one byte to each of the 39 newline-terminated records (`41,322` actual vs `41,283` expected). Added the exact fixture path as `-text` in `.gitattributes`; verified `git check-attr` reports `text: unset` and `git ls-files --eol` reports `attr/-text`; focused DeepSeek Harness tests pass 15/15 and GitHub Actions CI run `32260762462` completed success. Moved this plan to `completed/` and updated the active/completed plan index and tech-debt references. / 2026-08-19（跨平台 fixture 完整性／收尾）：确认 Windows `npm test` 失败原因是 Git 将物理未压缩 DSH `session.jsonl` 检出为 CRLF，39 条换行结尾记录每条增加 1 字节（实际 `41,322` vs 预期 `41,283`）。已在 `.gitattributes` 中把该精确 fixture 路径标记为 `-text`；验证 `git check-attr` 报告 `text: unset`、`git ls-files --eol` 报告 `attr/-text`；DeepSeek Harness 聚焦测试 15/15 通过，GitHub Actions CI run `32260762462` 全部成功。已将本计划移至 `completed/`，并更新 active/completed 计划索引与 tech-debt 引用。
...
- PARTIAL (local WSL rendering limit, non-application) — Playwright Chromium was installed under `tmp/ms-playwright`. With a local ignored WSL launch-args shim (`--disable-gpu --disable-software-rasterizer`; required because unmodified headless Chromium does not produce `requestAnimationFrame` frames in this sandbox), the browser suite is **116/117**. The three stale two-source chooser assertions were updated for DeepSeek Harness and now pass 3/3; the one remaining failure is the search-scroll guard wheel test, which times out on WSL wheel/rendering behavior. No product code was changed for these browser results. / PARTIAL（本地 WSL 渲染限制，非应用）——Playwright Chromium 已安装到 `tmp/ms-playwright`。使用本地忽略的 WSL 启动参数 shim（`--disable-gpu --disable-software-rasterizer`；必需，因为该 sandbox 中未修改的 headless Chromium 不产生 `requestAnimationFrame` frame）后，浏览器套件为 **116/117**。三个旧的仅两个来源 chooser 断言已按 DeepSeek Harness 更新并 3/3 通过；唯一剩余失败是 search-scroll guard wheel 测试，在 WSL wheel／rendering 行为下超时。这些浏览器结果没有修改任何产品代码。

- PASS — GitHub Actions CI after the cross-platform fixture fix: run `32260762462` at `89587b95fe93896455354850b7a1df9ff08e3eb7` completed success for Node 22/24 on Ubuntu, Node 24 on Windows, package smoke on Ubuntu/Windows, browser on Ubuntu, and aggregate `ci`. The final closeout commit `c382c072e631ddd5dc4c86c9c5b418c3e85a6044` also passed the full CI matrix in run `32261493322`. This closes the previously observed Windows-only failure in `uncompressed session.jsonl current-writer fixture works`; diagnosis confirmed as Git CRLF conversion of the physical uncompressed DSH fixture, fixed by adding that exact fixture path as `-text` in `.gitattributes`. / PASS — 跨平台 fixture 修复后的 GitHub Actions CI：run `32260762462` 在 `89587b95fe93896455354850b7a1df9ff08e3eb7` 全部成功，包括 Ubuntu Node 22/24、Windows Node 24、Ubuntu/Windows package smoke、Ubuntu browser 与聚合 `ci`。最终收尾提交 `c382c072e631ddd5dc4c86c9c5b418c3e85a6044` 也在 run `32261493322` 中通过完整 CI 矩阵。此前仅 Windows 出现的 `uncompressed session.jsonl current-writer fixture works` 失败已关闭；诊断确认为 Git 将物理未压缩 DSH fixture 检出为 CRLF，通过在 `.gitattributes` 中把该精确 fixture 路径标记为 `-text` 修复。
