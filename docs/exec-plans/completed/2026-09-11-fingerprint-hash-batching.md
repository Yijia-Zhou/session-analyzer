...
The focused local optimization experiment is complete and ready for review. No release is performed by this plan; merge remains maintainer-controlled. Evidence and limitations are recorded in [the measurement report](../../design-docs/deepseek-readback-measurement.md#equivalent-textual-hash-batching-experiment--等价文本-hash-批量写入实验). New raw data stays under main-checkout ignored `tmp/hash-batching-comparison/`; prior raw data remains preserved. / 聚焦本地优化实验已完成，可供评审。本计划不执行发布；合并仍由维护者控制。证据及限制见测量报告；新原始数据保留在主 checkout 的 ignored 目录，既有原始数据继续保留。
