...
- Status: active; repository workflow merged and maintainer-confirmed external trust configuration complete; first live staged release pending / 状态：进行中；仓库 workflow 已合入，维护者已确认外部 trust 配置完成；首次真实 staged release 待完成
...
- Related version plan: `docs/exec-plans/active/2026-08-02-v0.1.3-release.md`
...
- Run anonymous Windows/Linux public verification after approval, then create the exact Git tag and GitHub Release. / approve 后执行匿名 Windows/Linux 公共验证，然后创建精确 Git tag 与 GitHub Release。
...
7. Runbook, active plan, tests, and technical-debt status agree. / Runbook、active plan、测试与技术债状态保持一致。
