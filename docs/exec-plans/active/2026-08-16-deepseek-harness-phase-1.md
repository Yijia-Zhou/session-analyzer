...
Local toolchain: Node `v25.6.1`, npm `11.17.0`; the repository requires Node `^22.22.2 || ^24.15.0` and npm `12.0.2`, so these local results are advisory except for focused application tests. / 本地工具链：Node `v25.6.1`、npm `11.17.0`；仓库要求 Node `^22.22.2 || ^24.15.0` 与 npm `12.0.2`，因此除聚焦应用测试外，本地结果仅供参考。

- PASS — Focused DeepSeek Harness Node tests: `npm_config_force=true node --test test/deepseek-harness.test.js` → 15/15, including the new append/replacement source-freshness regressions and the 4,096-member packed-row no-expansion regression. / PASS — DeepSeek Harness 聚焦 Node 测试：15/15，包含新增 append／replacement 来源新鲜度回归与 4,096 成员打包行不展开回归。
- PASS — Source-adapter conformance: `npm_config_force=true node --test test/source-adapter-conformance.test.js` → 13/13. / PASS — source-adapter conformance：13/13。
- PASS — Source switch and build-boundary focus: `npm_config_force=true node --test test/source-switch.test.js test/build-boundary.test.js` → 28/28. / PASS — source switch 与 build-boundary 聚焦：28/28。
- PASS — Package smoke: `npm_config_force=true node scripts/package-smoke.js` completed Codex, Claude Code, and DeepSeek Harness rounds. / PASS — package smoke：Codex、Claude Code、DeepSeek Harness 三轮完成。
- FAIL (environmental, non-application) — Full Node suite: `npm_config_force=true node --test test/*.test.js` → 625/626. The sole failure is the known `test/package.test.js` npm-pack manifest assertion under npm `11.17.0` (`docs/README.md` is auto-included); the required npm `12.0.2` environment is not available here. No application test failed. / FAIL（环境性，非应用）——完整 Node 套件：625/626。唯一失败是已知的 `test/package.test.js` npm-pack manifest 断言，原因是 npm `11.17.0` 自动包含 `docs/README.md`；本环境没有仓库要求的 npm `12.0.2`。没有应用测试失败。
- FAIL (local generated-asset drift, non-application) — `npm_config_force=true npm run build:check` reports `public/vendor/highlightjs/highlight.min.js` and `public/assets/app.js` as stale under the local toolchain. No browser/generated source was changed by this follow-up; this is not treated as an application regression and the authoritative generated-asset gate is deferred to the required Node/npm environment. / FAIL（本地生成资产漂移，非应用）——`build:check` 在本地工具链下报告两个生成资产过期。本次后续未修改浏览器／生成源码；不视为应用回归，权威生成资产门槛留待要求环境。
- NOT RUN — Browser Playwright suite: the required Node/npm matrix and the Playwright Chromium browser are not available in this environment. / NOT RUN — 浏览器 Playwright 套件：本环境没有要求矩阵与 Playwright Chromium。
