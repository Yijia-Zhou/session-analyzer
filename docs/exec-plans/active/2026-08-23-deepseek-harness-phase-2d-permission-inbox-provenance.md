...
- Status: active; M1/M2 implementation and all applicable local closeout gates are complete; remote CI and final archival remain. / 状态：活跃；M1／M2 实现与全部适用本地收尾门禁已完成；尚待远程 CI 与最终归档。
...
- [x] M2 — strict inbox replay, exact relation metadata/event refs, optional splice Detail, defensive fallback tests, and exact 19-message gate. / 严格 inbox replay、精确 relation metadata／event ref、可选 splice Detail、防御回退测试与精确 19-message 门禁。
...
- 2026-08-23 (M2): strict child-owned two-list replay admits only exact enqueue／claim provenance; append-origin user messages require a unique claimed MessageId and exact full-message equality. User events retain one own Raw ref and receive relation IDs only; Detail exposes Supplemental KV plus `event_refs`. Cancellation/replacement remain generic, malformed replay disables later correlation, and inherited rows never attach to child events. Focused Phase 2D passes 9/9 after correcting one test-only 5-vs-6 splice-count typo; the affected DeepSeek／Detail／renderer／conformance gate passes 90/90. Six-artifact replay yields exactly 19 relations (10 human next-turn, nine plugin next-step), 38 independently owned splice rows, zero Raw collision, and unchanged Main／Code／Retry／Goal／Todo oracles. / 严格 child-owned 双列表 replay 只准入精确 enqueue／claim provenance；append-origin user message 必须具有唯一已 claim MessageId 与精确完整 message equality。User event 保留一条自有 Raw ref，只获得 relation ID；Detail 暴露 Supplemental KV 与 `event_refs`。Cancellation／replacement 保持 generic，畸形 replay 禁用后续关联，继承 row 不附着于 child event。修正一处仅测试的 5-vs-6 splice 数量笔误后，Phase 2D 聚焦测试 9／9 通过；受影响 DeepSeek／Detail／renderer／conformance 门禁 90／90 通过。六工件 replay 精确得到 19 relation（10 human next-turn、九 plugin next-step）、38 条独立 ownership splice row、零 Raw collision，Main／Code／Retry／Goal／Todo oracle 不变。
- 2026-08-23 (M3 local): full Node passes 705/705. `release:check` passes current generated assets, another 705/705 Node run, and Codex／Claude／DeepSeek package smoke. The first full browser attempt hit one unrelated 30-second source-home timeout and later hung in one search-navigation test until manually terminated; both exact tests immediately passed 2/2 in focused isolation. A fresh complete browser run then passed 121/121 and was not rerun again. Final six-artifact replay repeats the exact M1/M2 oracle, and `git diff --check` passes. Remote CI is not yet run. / 完整 Node 705／705 通过。`release:check` 通过 generated asset 检查、再次 705／705 Node 与 Codex／Claude／DeepSeek package smoke。首次完整 browser 尝试在一条无关 source-home 用例触发 30 秒超时，随后另一条 search-navigation 用例挂起直至手动终止；这两条精确用例随即隔离聚焦通过 2／2。重新进行的一次完整 browser run 随后通过 121／121，且未再次重跑。最终六工件 replay 再次满足精确 M1／M2 oracle，`git diff --check` 通过。远程 CI 尚未运行。
