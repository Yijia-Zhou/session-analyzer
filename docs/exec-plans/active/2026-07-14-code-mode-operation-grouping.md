...
- Status: active; design and execution plan opened, runtime implementation not started / 状态：进行中；设计与执行计划已建立，运行时实现尚未开始
...
| JavaScript loop, branch, and `Promise.all` call sites / JavaScript 循环、分支与 `Promise.all` 调用位置 | Declared summaries only; zero runtime count / 仅声明摘要；运行时计数为零 |
