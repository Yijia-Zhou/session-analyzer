...
- Status: active, M1 contract accepted but milestone-order reconciliation review is pending; no M2 production work started / 状态：活跃，M1 契约已接受，但 milestone 顺序对齐评审待完成；尚未开始 M2 生产工作
...
| M1 — Lifecycle architecture packet / 生命周期架构评审包 | Closed Indexed/Materialized/descriptor/dependency/query contracts, route matrix, data flow, scheduler/failure semantics, Codex/Claude sketches, compatibility, non-goals, review dispositions / 封闭 Indexed／Materialized／descriptor／dependency／query 契约、route 矩阵、数据流、scheduler／failure 语义、Codex／Claude 草图、兼容性、非目标、评审处理 | Contract accepted; sequence reconciliation review pending / 契约已接受；顺序对齐评审待完成 | M2 remains closed until the exact reconciliation head is accepted / 精确对齐 head 接受前，M2 继续关闭 |
| M2 — Lifecycle seam without memory change / 不改变内存的生命周期 seam | Discriminated `resident-complete-v1`/`indexed-materialized-v1` canonical/adapter seam; `materializeSession` required in both modes; both production adapters use compatibility materialization of their existing complete Sessions; conformance traverses Indexed → Materialized / 判别式两种 canonical／adapter seam；两种 mode 都要求 `materializeSession`；两个生产 adapter 对既有完整 Session 使用兼容物化；conformance 穿过 Indexed → Materialized | Pending / 待开始 | Focused lifecycle discrimination, ownership/equality, cancellation/non-mutation, descriptor validation, adapter dispatch/conformance, full Node gates; no user-visible or resident-memory change / 聚焦 lifecycle 判别、ownership／等值、cancellation／不可 mutation、descriptor validation、adapter dispatch／conformance、全量 Node 门槛；无用户可见或常驻内存变化 |
| M3 — Shared project-query independence / 共享项目查询独立 | Build/adopt one shared packed query store for both legacy adapters; move list/sort/project search/filter/suggestions/catalog/navigation off complete events; event APIs explicitly consume compatibility-materialized Sessions / 为两个 legacy adapter 构建／采用同一共享紧凑查询存储；列表／排序／项目搜索／filter／建议／catalog／导航脱离完整 event；event API 显式消费兼容物化 Session | Pending / 待开始 | Negative array-free Indexed fixtures, exact query oracle/cancellation gates, Luna max hidden-dependency sweep, then mandatory implementation checkpoint accepted before M4 / 无 event array 的 Indexed 负向 fixture、精确 query oracle／cancellation 门槛、Luna max 隐藏依赖扫描；M4 前强制实现检查点接受 |
| M4 — Codex source-backed Session / Codex 来源回读 Session | Switch Codex to Indexed build output; transient full parse, fork/query/summary derivation, source-backed materialization, detail/Raw/image parity, default-heap corpus evidence / 把 Codex 切换为 Indexed build output；瞬态完整解析、fork／query／summary 派生、来源回读物化、detail／Raw／image 等价、默认 heap 语料证据 | Pending / 待开始 | M3 checkpoint accepted first; Codex structural/fork/stale/cancel/reindex/conformance tests and real-corpus memory gates / 必须先接受 M3 检查点；Codex 结构／fork／stale／cancel／reindex／conformance 测试与真实语料内存门槛 |
| M5 — Claude source-backed Session / Claude 来源回读 Session | Switch Claude to the same lifecycle with interned positive/negative/directory dependencies and preserved primary/derived/pointer/materialized-fork semantics / 把 Claude 切换到同一生命周期，使用 intern 正向／负向／directory 依赖并保持 primary／derived／pointer／materialized-fork 语义 | Pending / 待开始 | Pairing/detail/analysis/derived/fork/reindex conformance, no reachable parsed graph, available-corpus memory gate; no legacy production adapter remains / pairing／detail／analysis／derived／fork／reindex conformance、parsed 图不可达、可用语料内存门槛；不再有 legacy 生产 adapter |
| M6 — Runtime materialization owner and integration / Runtime 物化 owner 与集成 | Revision-scoped unbounded cache, active-one/queued-32 scheduler, coalescing, cancellation, stale errors, source switch/replacement isolation, minimal browser recovery / Revision-scoped 无界 cache、active-one／queued-32 scheduler、合并、cancellation、stale error、来源切换／替换隔离、最小 browser recovery | Pending / 待开始 | Cold/warm/event/detail/Raw/image integration, replacement/cache-release, browser/build/package/full Node gates / 冷／热 event／detail／Raw／image 集成、替换／cache release、browser／build／package／全量 Node 门槛 |
...
| M1 milestone-sequence delta review, pass 4 / M1 milestone 顺序 delta 评审，第 4 轮 | Pending next immutable documentation head / 等待下一个不可变文档 head | Pending / 待定 | Must prove the restored sequence is executable without weakening accepted ownership/query/freshness contracts / 必须证明恢复后的顺序可执行，且不削弱已接受 ownership／query／freshness 契约 |
...
- M2 keeps both production adapters resident-complete but requires compatibility `materializeSession`, non-mutation/cancellation/identity validation, and conformance through that seam. / M2 让两个生产 adapter 继续 resident-complete，但要求兼容 `materializeSession`、不可 mutation／cancellation／identity validation，以及穿过该 seam 的 conformance。
- M3 exact query-oracle parity covers both sources/locales, array-free Indexed fixtures, asynchronous query cancellation, filtered navigation ordinal/revision, and explicit Materialized inputs for event-level APIs. / M3 精确 query-oracle 等价覆盖两种来源／locale、无 event array 的 Indexed fixture、异步 query cancellation、过滤后导航 ordinal／revision，以及 event-level API 的显式 Materialized input。
