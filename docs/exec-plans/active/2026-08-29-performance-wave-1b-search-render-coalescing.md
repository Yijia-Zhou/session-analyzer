...
- Status: `PASS_M3_CANDIDATE_READY_FOR_M4`; technical design, M1, M2, R1, R2, generated-asset inspection, and every M3 executor gate are accepted. The single candidate commit is the remaining M3 action immediately following this pre-commit record; M4 is authorized only from that clean exact candidate. / 状态：`PASS_M3_CANDIDATE_READY_FOR_M4`；技术设计、M1、M2、R1、R2、生成资产检查与全部 M3 executor gate 均已获接受。唯一 candidate commit 是紧随本 pre-commit record 的剩余 M3 action；M4 仅可从该干净且精确的 candidate 开始。
...
Completed milestones: M0, M1, M2; M3 executor gates complete
Independent reviews: R1 / PASS_M1; R2 / PASS_M2
M4 through M6: UNEXECUTED
Status: PASS_M3_CANDIDATE_READY_FOR_M4
```
