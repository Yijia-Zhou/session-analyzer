...
- Current observation: a 2026-07-30 read-only scan of 478 real active and archived Codex JSONL files found two exact previously unadmitted shapes. `view_image_tool_call` appeared 132 times in 9 files and was always mirrored by a same-file supported response-item call/output pair. `sub_agent_activity` appeared 871 times in 68 files at the frozen snapshot and continued growing; its `event_id`, `agent_thread_id`, and `agent_path` fields were stable. Later structural verification found 533 distinct event IDs, all matching real Agent Coordination call IDs somewhere in the corpus, while 340 of 873 occurrences were cross-Session replicas without a local owner. Synthetic coverage now protects the existing view-image mirror behavior and bounded same-Session subagent-activity correlation. Real dynamic tool, request-permissions approval, explicit hook lifecycle, and the other reviewed upstream-only lifecycle rows remain unobserved. A separate real transcript also confirmed the paired image-generation shape, and the 2026-07-13 upstream pass found optional paginated rollout ordinals plus terminal error/timing/source/process fields that still need focused fixtures. / 当前观察：2026-07-30 对 478 个真实活动与归档 Codex JSONL 文件进行只读扫描，发现两个此前未准入的精确形态。`view_image_tool_call` 在 9 个文件中出现 132 次，并且总是由同文件内受支持的 response-item call/output 配对镜像。固定快照中 `sub_agent_activity` 在 68 个文件中出现 871 次，随后仍持续增长；其 `event_id`、`agent_thread_id` 与 `agent_path` 字段保持稳定。后续结构复核发现 533 个不同 event ID，全部能在语料某处匹配真实 Agent 协调 call ID；而后续 873 条观察中的 340 条属于没有本地 owner 的跨会话副本。合成覆盖现在会保护既有 view-image 镜像行为与有界的同会话 subagent-activity correlation。真实 dynamic tool、request-permissions approval、显式 hook lifecycle 及其他已审查的仅由上游得知的 lifecycle row 仍未观察到。另一个真实 transcript 也已确认图像生成配对形态，而 2026-07-13 上游检查发现的可选分页 rollout ordinal 与终态 error/timing/source/process 字段仍需聚焦 fixture。
- Related docs: / 相关文档：
  - `docs/design-docs/logical-event-timeline.md`
  - `docs/design-docs/codex-protocol-event-coverage.md`
  - `docs/exec-plans/completed/2026-04-21-transcript-normalization-followups.md`
...
- Progress: the Code Mode detail-presentation increment and lifecycle-family item 1 are complete. The immutable lifecycle contract records all 42 admitted exact members and exact admission closes the bounded same-`callId` lookalike leak. The pinned upstream review initially kept every current unadmitted shape at `fixture TODO / coverage update`; a subsequent full local-corpus scan supplied the missing evidence for two shapes. The fully mirrored `view_image_tool_call` path now has explicit regression coverage without new semantic admission. High-frequency `sub_agent_activity` rows justified a separate bounded typed-correlation increment: exact same-Session observations may add traceability to an existing Agent Coordination owner without becoming lifecycle members or generic call-ID rows. / 进展：Code Mode detail 展示增量与 lifecycle-family 事项 1 均已完成。不可变 lifecycle contract 记录全部 42 个已准入精确成员，且精确准入关闭有界的 same-`callId` lookalike 泄漏。固定上游审查最初让每个当前未准入形态保持 `fixture TODO / coverage update`；后续全量本地语料扫描为其中两个形态补充了缺失证据。完全镜像的 `view_image_tool_call` 路径现在具有显式回归覆盖，但没有新的语义准入。高频 `sub_agent_activity` 记录支持建立独立、有界的带类型 correlation 增量：精确的同会话观察可以为既有 Agent 协调 owner 增加可追溯性，但不会成为 lifecycle member 或泛化 call-ID 记录。
- Direction: keep the completed subagent-activity correlation bounded without reopening lifecycle-family membership, cross-Session grouping, or Planning semantics. Keep all other current upstream-only shapes at fixture TODO until focused real evidence establishes product value; structured section-type consolidation remains deferred until a new type is needed. / 方向：继续保持已完成 subagent-activity correlation 的有界范围，不重新打开 lifecycle-family membership、跨会话 grouping 或 Planning 语义。其他当前仅由上游得知的形态继续保持 fixture TODO，直到聚焦真实证据明确产品价值；结构化 section 类型收敛继续推迟到确有新类型需求时。
- Completed typed-correlation plan: / 已完成带类型 correlation 计划：`docs/exec-plans/completed/2026-07-30-subagent-activity-correlation.md`
- Completed plan: / 已完成计划：`docs/exec-plans/completed/2026-07-30-tool-lifecycle-family-contract.md`
