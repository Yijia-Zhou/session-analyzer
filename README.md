...
With the current Indexed/Materialized lifecycle, an aggregate-only Codex run over 488 Sessions and 300,997 Raw Records completed under the default 2.35 GB V8 heap limit. It observed a 484 MB transient V8 heap peak and 1.90 GB process maximum RSS, then retained 55.7 MB V8 heap after forced GC alongside a 1.044 GB packed query store held primarily outside V8 heap. The largest accepted transcript in that run was 116.9 MB: cold materialization took 6.55 seconds, the exact warm cache hit took 0.3 ms without another adapter call, and the cached complete Session added about 44.0 MB heap and 15.6 MB external memory after GC. Treat histories approaching roughly 1 GB of matching transcript data as high process-memory workloads even though the source-backed lifecycle substantially reduces V8 heap pressure. These empirical measurements are guidance, not guarantees, a prediction formula, an out-of-memory boundary, or hard capacity limits; actual use varies with record shapes, event counts, Node version, the number of distinct Sessions opened in one revision, and unusually large individual Sessions.
